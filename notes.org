...
Alternatively, the formulas could be made symmetric by incurring a cost of $(o+e)/2$
to open a gap and a cost of $(o+e)/2$ when closing a gap:
\begin{align}
I_{s,k} &= \max\big\{M_{s\mathbf{\color{lime}-o/2-e/2},k-1}+1, I_{s-e,k-1}+1\big\}\\
D_{s,k} &= \max\big\{M_{s\mathbf{\color{lime}-o/2-e/2},k+1}, D_{s-e,k+1}\big\}\\
X_{s,k} &= \max\big\{M_{s-x,k}+1, I_{s\mathbf{\color{lime}-o/2-e/2},k}, D_{s\mathbf{\color{lime}-o/2-e/2},k}\big\}\\
M_{s,k} &= X_{s,k} + LCP\big(A[X_{s,k}-k \dots], B[X_{x,k}\dots]\big).
\end{align}


Note that this problem only arises for affine costs. The reason is the
following:
for linear costs, a path that goes through $n$ states has $n-1$ edges.
For affine costs, a path between two $M$ states with exactly one affine indel
has $n$ edges: there is one extra /free/ edge to /leave/ the affine layer, from
$I_{x,k}$ to $X_{s,k}$. In the reverse direction we must also ensure this edge
is free.

An alternative solution (that makes both path have length $n$ again)
would be to make $X_{s,k}$ depend on $I_{x-e,k-1}$ by /inlining/ one extend step
of $I$ into $X$. However, this has the drawback that gaps of size $1$ are not
possible anymore without further workarounds.
...
- Adaptive A* [cite:@adaptive-astar] ::
  ...
  shortest path to a fixed set of goal states, but varying star states. Input:
