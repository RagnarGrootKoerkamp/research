...
- /Pruning/ is an additional technique that penalizes searching behind the /tip/
  of the search. As soon as the start of a match is expanded, the match is not
  needed anymore and can be removed. This makes the heuristic inconsistent, but
...
This results in near-linear scaling ([[scaling]]) on aligning long sequences with low constant uniform error
...
*Status:* This work package is done and used for e.g. [[vis]]. Visualizations will be updated
...
- input data, which can be either random or human, and both can have different
  types of errors,
- error rate,
- sequence length,
...
benchmark A*PA. The comparison of tools is still pending.
...
- new recurrence relations have been found [cite:@difference-recurrence-relations],
...
heuristics, and how changing to an equivalent cost model can have an equivalent
effect to using a heuristic.
...
strings is more than $f$, and the band must grow. Ignoring pruning, this would
...
1. For each column $i$ store the last value of $f_i$, which starts at the value
   ...
   the last /increment/ $\Delta_i$ which starts at $1$.
2. For increasing $i$ starting at $0$, find the range of
   column $i$ with $f(u) \leq f_i$ and compute it efficiently using SIMD.
3. As soon as the range is empty for some column, /backtrack/ and double the band for
   ...
   2. When $f_{j} \leq f_{j-1}$, stop decreasing $j$.
...
values for each column still require a lot of overhead. Using /blocks of
columns/ as in block aligner [cite:@block-aligner] could significantly reduce
this, since then the bookkeeping is only needed every $8$ or $32$ columns.
...
applicability of A*PA can be generalized to affine gap-costs as well.

There are two parts to this. First, the underlying alignment graph needs to be
changed to include multiple /layers/, as introduced by [cite:@gotoh] and used by
[cite:@wfa]. This is straightforward and can be reused from the re-implementation
...
stronger and can correctly predict the long indels. This will make aligning
...
and changes the specific structure of the chaining seed heuristic (CSH) and gap-cost
chaining seed heuristic (GCSH) that we currently exploit for their relatively
...
the /layers/ (see [cite/t:@astarpa]) of the heuristic.
...
sketching [cite:@minhash]. Exact methods such as [cite/t:@lv89] are not used,
...
This open ended research could be on various topics.
 ...
 - A* for RNA folding ::
   ...
   number of bonds between complementary bases. Current exact methods tend to be
   slow and cannot efficiently handle sequences much longer than $10000$ bp.
 ...
 - Pruning A* heuristic for real-world route planning ::
   ...
   *Status:* Nothing more than the high level ideas described above.
...
# TODO: Add publications into this

# https://plantuml.com/gantt-diagram
#+begin_src plantuml :file time-schedule.png :eval no-export
@startgantt
hide footbox
