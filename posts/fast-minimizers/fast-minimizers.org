...
#+caption: Using a fixed-size vector drops the =grow= check! Runtime: =1.94ns/kmer= ↦ =1.77ns/kmer=.
#+begin_src rust
fn hash_kmers(&self, k: usize, t: &[u8]) -> impl Iterator<Item = Self::Out> {
    let len = t.len() - k + 1;
...
#+caption: Current assembly code for collecting ntHashes into a vector.
#+begin_src gas
...
This looks pretty great! The first three lines are somewhat redundant though,
since we know the unwrap can never fail[fn::Again, the compiler disappoints a
bit.].  And also on line 6, there is a 32-bit =mov= from =%r13d= to =%ecx= of
...
Either way, let's get rid of the last remaining branch:
#+caption: Using =unwrap_unchecked= to avoid a branch. Runtime: =1.77ns/kmer= ↦ =2.20ns/kmer=.
#+begin_src diff
 for x in v.iter_mut() {
-    *x = it.next().unwrap();
...
We've kinda spent enough time on this for now. But I'm still unhappy
