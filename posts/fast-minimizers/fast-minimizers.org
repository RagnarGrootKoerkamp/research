...
#+include: "code/blog/split.rs" src rust :lines "2-29"
...
thus aren't nearly as much of a bottleneck anymore. =perf stat= shows that there
are nearly no branch misses, and =2.43 IPC=, which is good but actually not
quite perfect yet.
...
#+caption: Reusing the =gather= at position $i+k$ for position $i$ when $k\leq 32$. Runtime goes from =0.17 ns/kmer= to =0.15 ns/kmer=.
#+begin_src diff
...
#+attr_shortcode: takeaway
#+begin_notice
We've gone from =2.9 ns/kmer= for the =nthash= crate to =0.17 ns/kmer= by using
32-bit hashes, a bit-packed input, and by using SIMD to parallellize over chunks
of the input. 17x speedup!
#+end_notice

** TODO Some more notes about ntHash

** TODO But what about a simple multiply hash

* TODO SIMD sliding window
