...
against each other. We will cover the benchmarking setup using =criterion= in detail in
[[*Setting up benchmarking]], and will then also compare the methods in more detail
for varying $k$ and $w$. Here, we use $k=21$, $w=11$, and measure the time it
takes to find the minimizers in a text of length $10^6$ characters.

#+caption: For /k=21/ and /w=11/, the naive buffered implementation runs in /55ms/ on a text of length 1 million characters, or /55ns/ per character. The three reported numbers are the low end of the confidence interval, the average measured time, and the high end of the confidence interval.
#+begin_src txt
g/Buffered              time:   [55.104 ms 55.115 ms 55.126 ms]
#+end_src
...
#+caption: For /w=11/, the =Queue= method is /2.5/ times faster than the =Buffered= method.
#+begin_src txt
g/Buffered              time:   [55.104 ms 55.115 ms 55.126 ms]
g/Queue                 time:   [22.831 ms 22.840 ms 22.849 ms]
#+end_src

One might expect a $w=11$ times speedup over the buffered method, but this is
not the case. This is due to the high number of unpredictable branches,
which hurts pipelining/instruction level parallelism (ILP). Thus, while the
complexity of the =Queue= algorithm is good, its practical performance is suboptimal.
...
#+caption: As predicted, =Jumping= is much faster than the previous methods, since it outputs less information.
#+begin_src txt
g/Buffered              time:   [55.104 ms 55.115 ms 55.126 ms]
g/Queue                 time:   [22.831 ms 22.840 ms 22.849 ms]
g/Jumping               time:   [8.0336 ms 8.0557 ms 8.0838 ms]
#+end_src
...
The result is quite good: almost twice as fast as the queue, although not as
fast as the =Jumping= method, which gets away with doing less work!

#+caption: =Rescan= is much faster than the =Queue= method, but not as fast as =Jumping=.
#+begin_src txt
g/Buffered              time:   [55.104 ms 55.115 ms 55.126 ms]
g/Queue                 time:   [22.831 ms 22.840 ms 22.849 ms]
g/Jumping               time:   [8.0336 ms 8.0557 ms 8.0838 ms]
g/Rescan                time:   [14.174 ms 14.176 ms 14.178 ms]
#+end_src
...
later, this causes a branch miss roughly every $w$ positions.
...
#+caption: The current implementation of =Split= is slightly faster than =Rescan=.
#+begin_src txt
g/Buffered              time:   [55.104 ms 55.115 ms 55.126 ms]
g/Queue                 time:   [22.831 ms 22.840 ms 22.849 ms]
g/Jumping               time:   [8.0336 ms 8.0557 ms 8.0838 ms]
g/Rescan                time:   [14.174 ms 14.176 ms 14.178 ms]
g/Split                 time:   [13.961 ms 13.962 ms 13.964 ms]
#+end_src
