...
1. An implementation of *32-bit ntHash* rolling hash that takes
   =100ps/kmer=[fn::100 picoseconds as in, 0.1 nanoseconds. At =2.6GHz= on
   an Intel =i7 10750H=. Slightly slower when $k>32$.], or 2 CPU cycles per 8
   hashes. This is *28x faster* than the [[https://crates.io/crates/nthash][=nthash=]] crate by [[https://twitter.com/luizirber][Luiz Irber]].
2. An implementation of *random minimizers* that takes =158ps/window=, or 3.3 CPU
   cycles per 3 windows. This is *60x faster* than [[https://twitter.com/daniel_c0deb0t][Daniel Liu]]'s [[https://gist.github.com/Daniel-Liu-c0deb0t/7078ebca04569068f15507aa856be6e8][gist]] and *160x
   faster* than the [[https://github.com/rust-seq/minimizer-iter][=rust-seq/minimizer-iter=]] crate by [[https://twitter.com/IgorMartayan][Igor Martayan]].

I plan to upstream these algorithms into the =rust-seq/nthash= and
=rust-seq/minimizer-iter= crates soon, after polishing of some implementation
details and designing a nice API[fn::On that matter, specific requests are welcome.].

I welcome any (critical) feedback on this post! I plan to fine-tune it a bit
more based on this, and will take this into account when writing similar posts
in the future. If you're feeling adventurous, open a pull request against the
[[https://github.com/RagnarGrootKoerkamp/research/blob/master/posts/fast-minimizers/fast-minimizers.org][source]] of this blog.
...
#+include: "code/blog/hash.rs" src rust :lines "-33"
...
#+include: "code/blog/hash.rs" src rust :lines "41-55"
...
#+include: "code/blog/hash.rs" src rust :lines "89-109"
...
ntHash already is only 'seeded' for DNA characters =ACTG=, and so the =[u64;
...
#+end_src
We wee that the it reports the maximum throughput of this block at one iteration
