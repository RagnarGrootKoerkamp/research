...
#+filetags: @paper @walkthrough hpc minimizers nthash software
#+HUGO_LEVEL_OFFSET: 0
#+OPTIONS: ^:{} num:2 H:4
...
#+hugo_aliases: /posts/fast-minimizers
#+date: <2024-07-12 Fri>


SimdMinimizers has been published as a paper: [[https://doi.org/10.1101/2025.01.27.634998][DOI]], [[../../static/papers/simd-minimizers.pdf][PDF]]:

#+begin_quote
[cite/bibentry/b:@simd-minimizers]
#+end_quote

This post is a more gentle walkthrough of the ideas in the paper.
A 90 min recording of a talk I gave on this post can be found [[file:../../static/talks/minimizer-talk.mp4][here]].
...
Code for this post is in the =simd-minimizers-bench/= directory of
[[https://github.com/rust-seq/simd-minimizers][github:rust-seq/simd-minimizers]].
...
2. An implementation of *random minimizers* and super-k-mers that takes around
   =1.6 ns/window=, or
   4 CPU cycles per window. This is *5x faster* than [[https://twitter.com/daniel_c0deb0t][Daniel Liu]]'s [[https://gist.github.com/Daniel-Liu-c0deb0t/7078ebca04569068f15507aa856be6e8][gist]] and *15x
   ...
   non-=ACGT= stripped) in *5.4 seconds*.
3. An implementation of *canonical minimizers* that runs in =1.9 ns/window=, or
   *6.4 seconds* for a human genome.

Some of the code to iterate bases is already available in the [[https://github.com/rust-seq/packed-seq][=packed-seq=]] crate
in the [[https://github.com/rust-seq][=rust-seq=]] org.
The remaining algorithms are planned to be upstreamed to =rust-seq/nthash= and/or
=rust-seq/minimizer-iter= crates, after polishing the implementation,
...
issue or pull request against the [[https://github.com/RagnarGrootKoerkamp/research/blob/master/posts/simd-minimizers/simd-minimizers.org][source]] of this blog.
...
computing sliding window minima (see also [[https://stackoverflow.com/questions/4802038/implement-a-queue-in-which-push-rear-pop-front-and-get-min-are-all-consta][this stackoverflow answer]] and an
implementation in the [[https://crates.io/crates/moving_min_max][=moving_min_max=]] crate).
...
When we now do =just bench=, we see this:
...
instructions]]:
- =_mm256_i64gather_epi32=, which takes a 256-bit SIMD register with four 64-bit
  indices and returns a 128-bit SIMD register with four 32-bit values.
- =_mm256_i32gather_epi32=, which takes a 256-bit SIMD register with eight 32-bit
...
#+caption: When using bit-packed data, we must read the position-=i= and position-=i+k= data separately. Runtime goes from =1.05 ns/kmer= to =0.62 ns/kmer=.
#+begin_src rust
let read = |i| {
...
They appear to be around =0.10 ns/kmer= slower than my original benchmarks, but
it is what it is.
#+end_notice
...
#+caption: Reusing the =gather= at position $i+k$ for position $i$ when $k\leq 32$. Runtime goes from =0.62 ns/kmer= to =0.57 ns/kmer=.
#+begin_src diff
...
jump in the main loop. That brings the runtime down to =0.57 ns/kmer=, or
...
#+caption: Checking whether $k\leq 32$ ahead of time saves a branch.chars_i= and =self.chars_k= variables in registers again. Runtime goes from =0.57 ns/kmer= to =0.52 ns/kmer=.
#+begin_src diff
...
#+caption: The new =Cached= hasher reuses the allocated buffer between usages. Runtime goes from =0.52 ns/kmer= to =0.39 ns/kmer=.
#+include: "code/blog/hash.rs" src rust :lines "176-"

Now, =perf= again shows that over 90% of time is spent in the =hash_kmers=
function, and indeed the runtime improves significantly: =0.39
ns/kmer=, or exactly one cycle per kmer![fn::Maybe it's time to switch to picoseconds (ps)?]
And also, we're at exactly =3 IPC= now, which at last seems quite reasonable.

#+attr_shortcode: takeaway
#+begin_notice
We've gone from =2.9 ns/kmer= for the =nthash= crate to =0.39 ns/kmer= by using
...
=0.35 ns/kmer=. But for consistency, it is simpler to always require an actual
vector as output.
...
relatively large portion of time is spent on the =gather= instructions that
occur only once every =32= iterations.
...
** Finals thoughts
Now that we've reached =0.39 ns/kmer=, and nearly 7.5x speedup over a previous
...
- =0.39 ns/kmer=
- =1.01 cycles/kmer=
- =8.08 cycles/8kmers=, i.e., 8 cycles per iteration of the main loop that produces 8
  hashes in a SIMD.
...
#+caption: Using FxHash instead of ntHash takes =0.32 ns/kmer=, down from =0.39 ns/kmer=!
#+begin_src rust
...
Again faster, with =0.32 ns/kmer= now, or 0.83 cycles per kmer!
...
2. Take a text, compute hashes to a (reused) vector, then read the vector to compute
   sliding window minima.
3. Like 2, but only measure the time for the sliding window minima.

As expected, method 2 reports close to =0.4ns= more than method 3, exactly the
...
that our returned iterators currently do not implement =ExactSizeIterator=, and
hence calling =Vec::extend(iterator)= may not be as efficient as it could be.
...
#+attr_shortcode: takeaway
#+begin_notice
We can now do sliding window minima in =0.63 ns/kmer=, or =1.6 cycles/kmers=.
That's 15x faster than =RescanDaniel=, and 40x faster than =QueueIgor=!
#+end_notice

*** Human genome results
For a human genome, consisting of roughly 3Gbp, we measure a runtime of =2 s=[fn::Compare that to the 40 seconds it takes to convert the byte-encoded
...
which corresponds to =0.66 ns/kmer=. Thus, the results we see transfer to
larger datasets without problems!

* Extending into something useful

(This section was added December 2024.)

** Collecting minimizer positions
At this point, we compute for every window the absolute position of its
minimizer. This is returned as a =Vec<Simd<u32, 8>>=, where the =8= SIMD lanes
correspond to separate chunks of the input.
In practice, we would prefer a simple =Vec<u32>=, without processing =8= lanes
...
elements into an =[Simd<u32, 8>; 8]=, corresponding to the next elements for
...
elements can be efficiently written to the output =Vec= in parallel.

The matrix transform code comes from [[https://stackoverflow.com/questions/25622745/transpose-an-8x8-float-using-avx-avx2][stackoverflow]] and looks like this.
#+caption: SIMD code to transpose a 8x8 matrix =[Simd<u32, 8>; 8]=, adapted from [[https://stackoverflow.com/questions/25622745/transpose-an-8x8-float-using-avx-avx2][this stackoverflow answer]].
#+begin_src rust
...
takes =1.3 ns/kmer=, compared to =1.0 ns/kmer= for the version that naively
collects into a =Vec<Simd<u32, 8>>=.

#+caption: Code to collect an iterator over =Simd<u32, 8>= into a =Vec<u32>=. Runs in =1.3 ns/kmer=, up from =1.0 ns/kmer= for collecting into a =Vec<Simd<u32, 8>>=.
#+begin_src rust
pub fn collect(
    ...
    par_head.for_each(|x| {
        m[i % 8] = x;
        if i % 8 == 7 {
            // Transpose the last 8 values into 8 lanes.
            let t = transpose(m);
            // Write each of the lanes to its own place.
            for j in 0..8 {
                unsafe {
...
** Deduplicating the minimizer positions
Now that we have a =Vec<u32>= of the minimizers positions of all windows, we
would like to deduplicate it, so that each unique position is listed only once.

For this, we adapt [[https://lemire.me/blog/2017/04/10/removing-duplicates-from-lists-quickly/][this post]] on Daniel Lemire's blog, which contains a function
that takes a =Simd<u32, 8>= of =new= values and a corresponding one of =old=
values (to decide whether the first element of =new= is distinct). It computes
which of the =new= elements are distinct from their previous element and appends
those to a vector.

#+caption: Code to extract the unique elements from a =Simd<u32, 8>=, adapted from [[https://lemire.me/blog/2017/04/10/removing-duplicates-from-lists-quickly/][this post]] on Daniel Lemire's blog.
#+begin_src rust
pub fn write_unique(
    old: __m256i,
    new: __m256i,
    ...
    unsafe {
        ...
        // Replace the highest index element of `new` with the one from `old`.
        let recon = _mm256_blend_epi32(old, new, 0b01111111);
        // Rotate lanes by one.
        let movebyone_mask = _mm256_set_epi32(6, 5, 4, 3, 2, 1, 0, 7);
        let prev = _mm256_permutevar8x32_epi32(recon, movebyone_mask);

        // Compare each lane of `new` to the `prev` element.
        let m = _mm256_movemask_ps(transmute(_mm256_cmpeq_epi32(prev, new))) as usize;
        let numberofnewvalues = WIDTH - m.count_ones() as usize;
        // This is a 256-long lookup table that stores which elements move where.
        let key = UNIQSHUF[m];
        // Move the distinct-from-prev elements of `new` to the front.
        let val = _mm256_permutevar8x32_epi32(new, key);
        // Append the full list of values to the output.
...
=Vec<u32>= and calls the function above for each 8 elements, which takes =1.6
ns/kmer=, up from =1.3 ns/kmer= for the version without deduplication.

In practice, it is slightly inefficient to first collect the transposed data
into a =Vec<u32>= that is only then deduplicated. Instead, we can use a
=[Vec<u32>; 8]= with a buffer for each lane, and deduplicate each lane directly
in =collect()=, before writing its results.
...
** Super-k-mers
We can now efficiently compute the list of all minimizer positions, but
unfortunately that information is not sufficient to exactly recover all
super-k-mers, i.e., all minimizers together with the interval of windows for
which they are minimal.
...
This turns out quite simple by slightly modifying the =write_unique()= function
...
We add an additional argument to the =write_unique= function, so that the mask
...
The returned values will now contain both the minimizer positions, and the
first windows where they occurred. Retrieving absolute positions rather than
mod-$2^{16}$ positions is easy by incrementing an offset whenever the index goes down.

This runs in =1.65 ns/kmer=, up from =1.60 ns/kmer= for simply returning
deduplicated minimizer positions without including super-k-mer information.

** Canonical k-mers

In most bioinformatics applications, canonical k-mers are used instead, that are
invariant under taking the reverse-complement of the sequence. This requires a
few modifications.

While the ideas in this section are generally known, the precise steps on
tiebreaking outlined
below were mostly suggested to me by Igor Martayan.


*** NtHash
First, we use the canonical version of NtHash that sums the hash of a kmer
and its reverse complement, so that it's invariant under reverse complement.
This requires duplicating most of the inner NtHash code, but is quite
straightforward. Since the stream of input characters only has to be read
once, this is less than 2x overhead.

This takes =1.83 ns/kmer=, up from =1.60 ns/kmer= for the version that does not
use canonical NtHash.

*** Leftmost-rightmost sliding min
When the minimum kmer hash occurs twice in a single window, we must make sure
that the two strands choose the same occurrence. To do that, we will track both
the leftmost /and/ rightmost occurrence of the minimum. (Other options are also
possible.)
...
the max of =(!hash, pos)=, alongside the existing minimum of =(hash, pos)=.

*** Tiebreaking
Lastly, we must consistently choose between choosing the leftmost and rightmost
occurrence.
We do this using the idea introduced by [cite/t:@refined-minimizer]: we count
the number of =AC= characters in a window, and say the window is /canonical/
...
Overall, this runs in =1.91 ns/kmer=, which is less than 2x slower than the =1.0
ns/kmer= we needed for simply collecting all (non-canonical) minimizer positions
...
Note that we do not require $k$ itself to be odd! Tiebreaking is an operation
that happens for a window, and hence only those need to have odd length.
...
- NtHash iterates over pairs of the character entering the new kmer, and the character
  leaving the new kmer, which reuses the same =gather= ([[*Reusing the gathers]]).
- The tiebreaking iterates over pairs of the character entering the new window (which is
  the same as the one entering the new kmer), and the character leaving the new
  window.
We would like to only do a single =gather= and use that for all three required positions.

Up to now, NtHash and the sliding minimum had a signature as in [[signature-old]], which
somewhat hinders this: since NtHash takes as input the input sequence directly,
and constructs the iterator over characters internally, we can't use it for the
tiebreaking iterator.
#+name: signature-old
#+caption: Signatures of NtHash and sliding minimum, taking ownership of an input iterator and returning a new iterator.
#+begin_src rust
fn hash_kmers(
    &mut self,
    k: usize,
    t: &[u8]
) -> impl Iterator<Item = [Self::Out; L]>;
...
Instead, we change our design to the following: the hash function and sliding
minimum simply return a closure with some internal state, and each time the
closure is called (with a pair of characters to be added and removed, or the new
hash), the
internal state is updated and the next hash or next sliding minimum is returned.
This turns out much simpler, and also optimized slightly better in some cases, [[signature-new]].
#+name: signature-new
#+caption: New signatures of NtHash and sliding minimum, returning a closure/'mapper' that takes elements one at a time.
#+begin_src rust
/// Returns a function taking pairs (a, r)
/// of characters added and removed from the last *kmer*,
/// that returns the new hash.
pub fn nthash_mapper(
    k: usize,
    w: usize
) -> impl FnMut((S, S)) -> S + Clone;

/// Returns a function taking new hashes,
/// and returning the new sliding window minimum.
pub fn sliding_min_mapper(
    w: usize,
    len: usize
) -> impl FnMut(S) -> S;
...
/// Returns a function taking pairs (a, r)
/// of characters added and removed from the last *window*,
/// that returns whether the new window is canonical
/// (when #AC is more than half).
pub fn canonical_mapper(
    k: usize,
    w: usize
) -> impl FnMut((S, S)) -> S;
#+end_src

For example, the =canonical_mapper= looks like [[canonical-mapper]]. Note though
...
#+caption: Function that initializes the 'canonical mapper' that tracks whether a window is canonical.
#+begin_src rust
/// NOTE: First l-1 returned values are bogus.
pub fn canonical_mapper(k: usize, w: usize) -> impl FnMut((S, S)) -> S {
    let l = k + w - 1;
    assert!(
        l % 2 == 1,
    ...
    // Count of odd characters, offset by -l/2 so >0 is canonical and <=0 is not.
    let mut cnt = Simd<i32, 8>::splat(-(l as i32) / 2);
    let zero = Simd<i32, 8>::splat(0);
    let one = Simd<i32, 8>::splat(1);

    // (a, r): character added and remvoed.
    move |(a, r)| {
        cnt += a & one;
        cnt -= r & one;
        cnt.cmp_gt(zero)
...
returning triples of characters =(new, last-in-kmer, last-in-window)=, and map
them as in [[mapper]].

#+name: mapper
#+caption: The canonical minimizer can be implemented cleanly using 'mapper' functions for NtHash, the sliding minimium, and the canonical determination.
#+begin_src rust
// Iterate characters, with a 'trailing' char `k-1` and `l` behind.
// `k-1` is first char of the kmer.
// `l` is the one to be dropped from the window.
let (char_triples, tail) = seq.par_iter_bp_delayed_2(k + w - 1, k - 1, l);

// Canonical nthash
let mut nthash = nthash_mapper::<true>(k, w);
// True/false whether a window is canonical.
let mut canonical = canonical_mapper(k, w);
// Leftmost and rightmost window minimum position.
let mut sliding_min = sliding_lr_min_mapper(w, char_triples.size_hint().0);

let mut iterator = char_triples.map(move |(a, rk, rl)| {
    let nthash = nthash((a, rk));
    let canonical = canonical((a, rl));
    let (lmin, rmin) = sliding_min(nthash);
    // Choose leftmost or rightmost min based on whether window is canonical.
    canonical.blend(lmin, rmin)
});
#+end_src

To implement the =delayed_2= iterator that returns triples of characters, the
new one and two 'delayed' streams, we extend the idea from [[*Reusing the gathers]]:
instead of storing the last read =gather= in a temporary variable, we write all
gathered data into a =Vec<S>= ring buffer that is long enough to cover the
...
In my [[../practical-minimizers][practical minimizers]] post, I introduce the 'anti-lexicographical'
...
character of the string (as in =A <-> Z= or =0 <-> 3=) and then doing a normal comparison.

The benefit of this method is that unlike NtHash, it does not require a table
lookup, and thus avoids some of the more complicated SIMD instructions.

I have not yet implemented and tested a reverse-complement variant of this, but
the forward version takes =1.40 ns/kmer= to collect and deduplicate, compared to
=1.55 ns/kmer= for forward NtHash.

* Conclusion
We now have a very optimized SIMD implementation to compute canonical minimizers,
taking under =2 ns/kmer= (or =5 cycles/kmer=).

#+name: summary
#+caption: Summary of the final timings for parts of the final algorithm.
#+attr_html: :class small
|                            | time (ns/kmer) |
| NtHash (sum)               | =0.39=         |
...
** Future work
Next up is to find some usage for this library, for example in my Rust
implementation of SSHash.
Additional extensions are possible of course:
- Implement and test a canonical version of =AntiLex=.
- Generalize the matrix-transpose and deduplication SIMD code to ARM and NEON
  instruction sets.
- In general, implement and test other low-density schemes such as the mod-minimizer.
Also, I think it has not been much investigated how general low-density schemes
can be transformed in to a canonical version, and how well they then perform.
