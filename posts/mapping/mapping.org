...
step after /global alignment/ (\cref{ch:pa}).
...
#+attr_latex: :scale 1.2 :placement [t]
[[file:fig/alignment-modes.svg]]

As we saw in \cref{ch:astarpa}, there are many different types of pairwise alignment ([[types]]).
...
See e.g.
[cite:@ukkonen85-patterns;@landau-vishkin89;@chang92;@baeza-yates-gonnet92;@wu92;@alpern95;@baeza-yates96;@hyyro05-increased;@rognes11] for a
...
#+attr_html: :class inset large
#+attr_latex: :scale 1.7 :placement [t]
[[file:fig/search.svg]]

In A*PA2 (\cref{ch:astarpa2}), we developed a bloc\(k\)-based method for pairwise alignment.
...
#+attr_html: :class inset medium
#+attr_latex: :scale 1.5 :placement [t]
...
#+attr_html: :class inset medium
#+attr_latex: :scale 2.0 :placement [t]
...
be found in a 1 kbp read in 13 micro seconds (75000 searches per second) or in a
50 kbp text in 440 micro seconds (2200 searches per second). Or alternatively, in one
...
*Minimizers.*
The most popular mapper, minimap2 [cite:@minimap2], uses /minimizers/ (\cref{pt:mini}). By
...
[cite:@astarpa] (\cref{ch:astarpa}), plain \(k\)-mer matches are used. A drawback of this
...
global alignment methods discussed in \cref{ch:astarpa}, such as a plain
...
** Results
In \cref{tab:map-results}, we compare A*Map against minimap2 on synthetic long read data.
...
could probably be sped up by using SimdMinimizers (\cref{ch:simdmini}).
