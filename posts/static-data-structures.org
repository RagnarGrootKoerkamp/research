...
We consider various variants of this setting.

Terminology:
- /function/: only defined on $K$
- /dictionary/: detects non-keys
- /retrieval/: static key-set
...
In the following, space usage ignores lower-order terms, and we do not consider
...
- Examples:
  - 3-way hypergraph peeling: $1.23v$ bits/key, $O(1)$ query via 3 parallel reads [cite:@hypergraph-peeling]
  ...
  - BuRR (bumped ribbon retrieval): $v$ bits/key, $O(v)$ query time ($v$ times $3$ parallel reads) [cite:@burr]
...
- Examples:
  ...
  - cuckoo hashing: $(1+\varepsilon)k$ bits/key; $O(1)$ queries with either 2
...
- Examples:
  ...
  - cuckoo hashing [cite:@cuckoo-hashing]: $(1+\varepsilon)(k+v)$ bits/key
  - optimal static dictionary [cite:@static-dictionary]: $n^\varepsilon$ overhead to lowerbound, $O(1)$ queries
...
| data structure/method          | non-keys | values  | space (bits/key)                        | query                           |
| *MPHF*                         | no       | -       | $\geq \log e$                           |                                 |
