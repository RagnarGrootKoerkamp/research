...
#+filetags: @survey data-structure mphf highlight
#+OPTIONS: ^:{} num: num:t
...
| monotone MPHF | x    |        |        |        | $f(x) = \vert\{x'\in K \mid x' < x\}\vert$    |
| static retrieval     | x    |        |        |        |                                                   |
...
** TODO Monotone MPHF
- MPHF that maps keys to their index in the sorted list.
- Space lower bound: TODO
- Examples: TODO
** TODO Order-preserving MPHF
- MPHF that maps keys to $[n]$ in the order as given in the input.
- Not very interesting, because the space lower bound is $\lg n$ bits per
  element, and so an MPHF followed by explicit values is near-optimal.
