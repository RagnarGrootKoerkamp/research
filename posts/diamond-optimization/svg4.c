...
bool copy_stack(struct stack* src, struct stack* dest) {
	if(dest->max_len < src->max_len) return false;
	int i;
	// make memcpy instead of a for loop
...
void generate_rect_with_anim(struct point pos, int stroke_width, int width, int height, char* color,
                             int steps, int cur_frame, FILE* fd) {
	char values[10000]    = "";
	...
	if(cur_frame == 0) {
		...
		sprintf(key_times, "0;%.5lf;%.5lf;1", ((double)(cur_frame)) / steps,
		        ((double)(cur_frame + 1)) / steps);
	}
	char str[10000];
	sprintf(str,
	        "<rect x=\"%d\" y=\"%d\" width=\"%d\" height=\"%d\" fill=\"%s\" stroke=\"#000000\" "
	        "stroke-width=\"%d\" pointer-events=\"all\">\n<animate id=\"frame1\" "
	        "attributeName=\"display\" values=\"%s\" keyTimes=\"%s\" dur=\"%lfs\" begin=\"0s\" "
	        "repeatCount=\"indefinite\" />\n</rect>\n",
	        pos.x, pos.y, width, height, color, stroke_width, values, key_times, duration);
	fputs(str, fd);
}

void generate_rect(char* str, struct point pos, int stroke_width, int width, int height,
                   char* color, int steps, int cur_frame, FILE* fd) {
	str[0] = 0;
	sprintf(str,
	        "<rect x=\"%d\" y=\"%d\" width=\"%d\" height=\"%d\" fill=\"%s\" stroke=\"#000000\" "
	        "stroke-width=\"%d\" pointer-events=\"all\"/>\n",
...
void draw_squares(struct stack* nodes, char* color, FILE* fd, FILE* fd_anim, int steps,
                  int cur_frame) {
	int i;
	char str[10000];
	if(nodes) {
		for(i = 0; i < nodes->size; ++i) {
			...
			generate_rect(str, get_coord((nodes->data)[i]), 4, SIDE, SIDE, color, steps, cur_frame,
			              fd_anim);
			fputs(str, fd);
		...
		for(i = 0; i < dist; ++i) {
			...
			for(k = 0; k <= 2 * i; ++k) {
				if(was_drawn(a)) {
					...
					generate_rect(str, get_coord(a), 4, SIDE, SIDE, color, steps, cur_frame,
					              fd_anim);
					fputs(str, fd);
				}
				++a.d;
...
void draw_image(struct stack* current, struct stack* dependency, struct stack* explored,
                struct stack* highlighted, const char* name, int file_number, FILE* anim, int steps,
                int cur_frame) { // if node.cost == -1 => end of the list
	char file_dir[1000] = "";
...
void opt_generate_rect_with_anim(int stroke_width, int width, int height, struct node a, int steps,
                                 FILE* fd) {
	...
	sprintf(str,
	        "<rect x=\"%d\" y=\"%d\" width=\"%d\" height=\"%d\" fill=\"%s\" stroke=\"#000000\" "
	        "stroke-width=\"%d\" pointer-events=\"all\">\n"
	        "<animate id=\"frame1\" attributeName=\"fill\" values=\"%s\" keyTimes=\"%s\" "
	        "dur=\"%lfs\" begin=\"0s\" repeatCount=\"indefinite\" />\n"
	        "</rect>\n",
	        pos.x, pos.y, width, height, color, stroke_width, values, key_times, duration);
	fputs(str, fd);
}
...
void diamond(int steps) {
	...
	for(int k = 0; k < dist - abs(d0); ++k) {
		...
		for(int i = k0; i > d0; --i) {
			...
			draw_image(&cur, &dep, &expl, &highlighted, __FUNCTION__, num, anim_file, steps, num);
			++num;
	...
	draw_image(NULL, NULL, &expl, &highlighted, __FUNCTION__, num, anim_file, steps, num);
...
void diamond_parallel_mem_save(int steps) {
	...
	for(int k = 0; k < dist - abs(d0); ++k) {
		...
		draw_image(&cur, &dep, NULL, &highlighted, __FUNCTION__, num, anim_file, steps, num);
		++num;
	}
	draw_image(NULL, NULL, &cur, &highlighted, __FUNCTION__, num, anim_file, steps, num);
	end_animation(anim_file, __FUNCTION__, steps);
}
...
void WFA_mem_save(int steps) {
	...
	for(int i = 0; i < dist; ++i) {
		for(int k = -i; k <= i; ++k) {
			...
			draw_image(&cur, &dep, &expl_prev, &highlighted, __FUNCTION__, num, anim_file, steps,
			           num);
			++num;
	...
	draw_image(NULL, NULL, &expl_prev, &highlighted, __FUNCTION__, num, anim_file, steps, num);
	end_animation(anim_file, __FUNCTION__, steps);
}
...
int main() {
	printf("Enter folder for images (for example C:\\images\\WFA\\; do not forget to change "
	       "slashes for Linux!): ");
	scanf("%s", path);
