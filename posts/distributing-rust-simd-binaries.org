...
See also a newer post on my [[./release-flow/release-flow.org][release setup]].

* What's inside
Initially, most of my code uses Rust's unstable =#![feature(portable-simd)]=.
...
features we need. Instead, we can force to use all features available on the host
