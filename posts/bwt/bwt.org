...
Instead, observe that the $Occ$ array we already have allows us to count the
...
\begin{align}
s' =& s + (\overline{Occ}[c-1][t'] - \overline{Occ}[c-1][s'])\\
t' =& s + (\overline{Occ}[c][t'] - \overline{Occ}[c][s']),
\end{align}
