...
and the new mod-minimizer (section [[*NEW: Mod-minimizer]]) achieve optimal density $1/w$.
...
* Sampling schemes
NOTE: The classification into groups of schemes is somewhat arbitrary.
...
Bruijn graph of order $k$ and remove all nodes in the MDS from it, this leaves a
...
can be reached, for example by the Mykkeltveit MDS $\Dk$ [cite:@mykkeltveit].

To construct $\Dk$, k-mers are first embedded into the complex plane via a
character-weighted sum of the $k$ $k$'th roots of unity $\omega_w$: a k-mer $X$ is mapped
to $x=\sum_i X_i\cdot\omega_w^i$.
This way,
...
It starts by using Mykkeltveit's MDS such that every infinitely long
...
*$\boldremuval$* [cite:@practical-uhs] is a method that build on DOCKS.
Given a UHS $U$ for some $(w, k-1)$, starting with a UHS generated by DOCKS, it
...
smallest k-mer in a $w+k$ long context is at the start or end. Thus, the
greedymini finds the k-mer that occurs relatively the least at the start or end
of a context and chooses that as the smallest k-mer. It then discards all
contexts this k-mer appears in and repeats the process until a minimizer has
been determined for all contexts. To improve the final density, slightly
submoptimal choices are also allowed, and a local search and random restarts are
used.
...
The resulting schemes achieve very low density, especially when $k$ is around
...
TODO: Merge with section above; sort chronologically.
...
the first scheme that is proved to break the $2/(w+1)$ hypothesis when $k\approx
w$.
...
#+begin_newtheorem Mod-minimizer is sometimes optimal
