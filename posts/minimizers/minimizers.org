...
memory and speed. And worse, in practice, most schemes do not give more than a
...
assumptions, and end with a new nearly tight lower bound.
...
minimizer and the general /mod-minimizer/. The most important result is that the
mod-minimizer achieves density $1/w$ in the limit where $k\to\infty$, and that
the convergence is close to optimal.

Lastly, in Section [[*Selection schemes]], we consider the special case where $k=1$.
Here, we first introduce the /bd-anchor/ [cite:@bdanchors-esa], and then improve
