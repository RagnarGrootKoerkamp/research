...
* TODO Experiments
Here are some quick results. This needs some more polishing though.
...
- TODO: Try both $\sigma = 4$ and $\sigma = 256$.
- TODO: Plot some lines at density $1.5/w$.
- TODO: Plot more k (and w?)
- TODO: Plot value of $k_0$ / $r$ that achieved the minimum.
...
- bd-anchors depend only on $\ell = w+k-1$, and hence density decreases in $k$.
  - It's cool to see that /non-local/ schemes can indeed break the $1.5/w$ lowerbound.
