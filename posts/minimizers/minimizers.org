...
schemes exactly [cite:@improved-minimizers Lemma 4;@miniception]:
...
sets and minimizer schemes [cite:@improved-minimizers Section
3.3;@asymptotic-optimal-minimizers Section 2.1.5;@small-uhs]:
...
smallest $k$-mer. Thus, we obtain [cite/t:@asymptotic-optimal-minimizers Theorem
2]:
...
Theorem 1] to get $2/(w+k)$ (see section [[#near-tight-lb]]). For local schemes, Remark 7 applies and with $k' = \max(k,3)$ we
...
But this is still not optimal: reduced bd-anchors [cite:@bdanchors Lemma 6] (see section
...
[cite:@modmini Corollary 17;@oc-modmini Theorem 2].
