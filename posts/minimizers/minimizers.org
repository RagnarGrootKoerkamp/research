...
consider a version with tiebreaking, /ABB+/, that first compares k-mers via ABB order,
and in case of ties compares them using the plain lexicographic order.
...
To end, we once more compare all sampling schemes so far in [[summary]]. We also include a
...
schemes as good as forward schemes, given that $k$ is sufficiently large?
