...
we now list some specific Rust (re)implementations of rank structures.
...
implementation. BiRank is usually slightly faster than our reimplementation of SPIDER.
...
This is because BiRank requires only a single cache miss per query, compared to
