...
:END:
In anticipation of the evaluations,
we list some specific Rust (re)implementations of rank structures.
...
#+name: cache-misses-2
#+caption: Average number of last-level cache misses per query on an input of 4 GiB, for alphabet size 2.
...
#+name: cache-misses-4
#+caption: Average number of last-level cache misses per query on an input of 4 GiB, for alphabet size 4.
#+attr_html: :class small
