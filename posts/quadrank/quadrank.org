...
and (3) an additional zeroth layer [cite:@poppy] that is modified to be
...
Both the [cite:@poppy] and [cite:@pasta] contain a nice
overview as well.
...
*Terminology.*
For later reference, we summarize our terminology, mostly following [cite:@poppy] and [cite:@pasta].
...
[cite:@rank-select-theory-practice], which numbers the layers bottom-up instead
of top-down as we do:
The root has all superblocks (L1) as children,
each superblock has its contained blocks (L2) as children,
...
overhead again, to 1.6%. Compared to PastaWide, instead of storing 16-bit (L2)
...
PastaFlat structure [cite:@pasta] (omitting the L0 layer), and =RSNarrow=,
...
#   - The new theory-practice paper reverses the numbering of layers. Consider doing the same here.
