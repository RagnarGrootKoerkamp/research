...
9. <<lcsk>> LCS$k$ is a variant of LCS where only runs of exactly $k$ consecutive equal
...
[cite/text/f:@four-russians], and may have been named as such since all authors
...
in the first subquadratic algorithm for edit distance. Briefly, they partition
the $n\times m$ matrix in blocks of size $r\times r$, for some $r=\log_k n$, as
shown in figure [[fig:four-russians]].
Using this, they speed up the computation of the DP by doing a single $O(r)$
lookup for each of the $O(n^2/r^2)$ blocks, for a total runtime of $O(n^2/\lg
n)$.

Assuming a discrete cost function, the number of essentially distinct states
along the top row and left column of each block (up to adding a constant) is
bounded by $c^{2r}$ for some $c$, and hence there are a total of $c^{2\log_k n}$
distinct blocks, which is $O(n^{1/2})$ for an appropriate choice of $k$. For each of
these blocks we can precompute the corresponding new states on the bottom row
and rightmost column in $O(n)$ total time. When filling the DP table, we can
then simply lookup the 'input' of each block to get the corresponding 'output'.
