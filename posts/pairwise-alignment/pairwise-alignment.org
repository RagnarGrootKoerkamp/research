...
#+caption: Overview of different alignment types.
...
- *Local-extension:* Align a prefix of the two sequences. Unlike all previous
  ...
  not matching characters at all.
...
- *Clipping:* Clipping is a mix of semi-global and local alignment.
  In principle, a semi-global alignment is done, but the read (shorter sequence)
  that is being aligned may be /clipped/. This means that any number of
  characters at the start or end of the read can be skipped for a fixed cost.
  As with local alignment, this usually requires maximizing a score, with a
  bonus for matches.
...
#+caption: Overview of different cost models.
#+name: cost-models
#+attr_html: :class inset medium
...
#+name: a1
#+caption: Schematic overview of global pairwise alignment methods.
#+attr_html: :class inset large
[[file:fig/algs-1.svg]]

#+name: a2
#+caption: Worked example of each algorithm, based on [cite:@sellers].
#+attr_html: :class inset large
[[file:fig/algs-2.svg]]
