...
seed heuristic is a direct application of his earlier work on A*ix.
Pesho Ivanov's contributions predominantly
include the extension of the heuristic to the more general seed heuristic with chaining, gaps,
...
*Overview.* This survey focuses on /exact/ /global/ pairwise alignment. In
particular, we focus on research from roughly 1970 to 1990 on
improving the complexity of alignment algorithms. This is followed by more
recent papers on improving the practical performance of these methods through
more efficient use of the hardware.
Rather than presenting all work chronologically, we treat them topic by topic.

*Scope.*
There is also a vast literature on /text searching/, where all occurrences of a
short pattern in a long text must be found. This field was very active from
around 1990 to 2010, and again includes a large number of papers.
...
reads. Here, we refer the reader to the survey by Alser at al. [cite:@alser21].

Lastly, we note that most moderns read mappers and alignment tools are
...
In A*PA [cite:@astarpa] (Section FIXME) we will improve this into the
...
#+end_quote
In A*PA2 [cite:@astarpa2] (Section FIXME),
...
2. $\{u: \g(u)\leq t\}$, the states at distance $\leq t$, introduced by
   Fickett [cite:@fickett84] and similar to Dijkstra's algorithm ([[intro]]ab) [cite:@dijkstra59].
...
* Some tools
We now briefly list a few recent and competitive global alignment tools. For more, we refer to the
survey of Alser et al. [cite:@alser21].
...
*WFA* [cite:@wfa] builds on the diagonal transition method
[cite:@ukkonen85;@myers86], and extends it to affine costs using a method
similar to [cite:@gotoh]. *BiWFA* [cite:@biwfa] is a later version that applies
divide-and-conquer to reduce to linear memory usage.
...
| [cite/text/cf:@nw]                                   | *edit distance* TODO ARBITRARY GAPS??? | $O(n^2m)$                                 | $O(nm)$                     | DP                                                                | solves pairwise alignment in polynomial time                                     |
...
| [cite/text/cf:@four-russians-ed]                     | edit distance                              | $\boldsymbol{O(n\cdot \max(1, m/\lg n))}$ | $O(n^2/\lg^2 n)$            | four-russians                                                     | best known complexity; requires finite alphaet                                   |
| [cite/text/cf:@smith81]                              | *affine*                                   | -                                         | -                           | -                                                                 | First to suggest affine, in future work.                                         |
| [cite/text/cf:@gotoh]                                | *affine*                                   | $O(nm)$                                   | $O(nm)$                     | DP                                                                | extends [cite/text:@sellers] to affine                                           |
| [cite/text/cf:@altschul]                             | affine                                     | $O(nm)$                                   | $O(nm)$                     | DP                                                                | Fixes bug in traceback of [cite:@gotoh]                                          |
...
| [cite/text/cf:@wilbur-lipman-83;@wilbur-lipman-84]   | LCSk                                       | -                                         | -                           | *chaining k-mer matches*                                          | Approximate                                                                      |
| [cite/text/cf:@fickett84]                            | Edit distance                              | $O(nt)$                                   | $O(nt)$                     | *Bound $\g(u)\leq t$                                              | Assuming $t\geq s$.                                                              |
| Dijkstra [cite:@dijkstra59;@ukkonen85;@myers86]      | Edit distance                              | $O(ns)$                                   | $O(ns)$                     | *Dijkstra's algorithm*                                            | Implement using $O(1)$ bucket queue                                              |
| [cite/text/cf:@ukkonen85]                            | edit distance                              | $\boldsymbol{O(ms)}$                      | $O(ns)$                     | band doubling                                                     | first $O(ns)$ algorithm for edit distance                                        |
| [cite/text/cf:@ukkonen85]                            | edit distance                              | $O(n+s^2)$ expected                       | $\boldsymbol{O(n+s^2)}$     | *diagonal transition*                                             | introduces diagonal transition method, requires fixed indel cost                 |
| [cite/text/cf:@myers86]                              | LCS                                        | $O(n+s^s)$ expected                       | $O(s)$ working memory       | *diagonal transition*, divide-and-conquer                         | introduces diagonal transition method for LCS, $O(n+s^2)$ expected time          |
| [cite/text/cf:@myers86]                              | LCS                                        | $\boldsymbol{O(n +s^2)}$                  | $O(n)$                      | suffix tree                                                       | better worst case complexity, but slower in practice                             |
...
| [cite/text/cf:@spouge89]                             | edit distance                              | -                                         | -                           | *A**, *computational volumes*                                     | Review paper                                                                     |
| [cite/text/cf:@gotoh90]                              | *double affine/piecewise-linear*           | $O(Lmn)$                                  | $O(nm+Lm)$                  | DP, *$L$ layers in the graph*                                     |                                                                                  |
| [cite/text/cf:@wu90-O-np]                            | unit cost                                  | $O(n+(s-\vert n-m\vert)s)$ expected       | $O(n)$                      | Diagonal transition, gap-heuristic, divide-and-conquer            |                                                                                  |
| [cite/text/cf:@sparse-dynamic-programming-1]         | LCSk                                       | $O(n + d \log\log \min(d, nm/d))$         |                             | *sparse-dynamic-programming*, contours                            | $d$ is number of /dominant/ matches                                              |
| [cite/text/cf:@myers-miller-95]                      | LCSk, edit distance                        | $O(r \log^2 r)$                           | $O(r \log r)$               | *chaining* k-mer matches with *gap-cost*                          | $r$ is number of matches                                                         |
| [cite/text/cf:@myers99]                              | unit costs                                 | $O(nm/w)$                                 | $O(m\sigma / w)$            | DP, *bitpacking*                                                  |                                                                                  |
| [cite/text/cf:@papamichail2009]                      | unit costs                                 | $O(n+(s-\vert n-m\vert)s)$                | $O(s)$                      | A*, gap heuristic, diagonal transition                            |                                                                                  |
| [cite/text/cf:@lcsk-overview]                        | LCS$k$                                     | $O(n + r \log l)$                         | $O(n + \min(r, nl))$        | thresholds                                                        | modifies [cite/text:@hunt77] for LCS$k$                                          |
| BitPAl [cite/text/cf:@bitpal]                        | Edit distance                              | $O(znm/w)$                                | $O(znm/w)$                  | Bitpacking, *difference recurrence*                               | $z$ depends on edit costs                                                        |
| [cite/text/cf:@grabowski14]                          | unit cost/LCS                              | $O(nm \log \log n / \log^2 n)$            | $o(n)$ overhead             | Four-russians                                                     | General alphabet                                                                 |
| Edlib [cite/text/cf:@edlib]                          | unit costs                                 | $O(ns/w)$                                 | $O(n)$                      | Bitpacking, band-doubling, divide-and-conquer                     | extends Myers' bit-packing to global alignment                                   |
| libgaba [cite/text/cf:@suzuki-kasahara]              | Affine                                     | -                                         | -                           | *SIMD*, *affine difference recurrence relation*                   | Adaptive band; not exact                                                         |
| KSW2 [cite/text/cf:@minimap2]                        | Affine, Double affine                      | $O(nm/w)$                                 | $O(nm/w)$                   | Implements [cite:@suzuki-kasahara]                                | $w$ SIMD lanes                                                                   |
| WFA [cite/text/cf:@wfa]                              | affine                                     | $O(n+s^2)$ expected                       | $O(n+s^2)$                  | diagonal-transition                                               | extends diagonal transition to gap affine [cite/text:@gotoh]                     |
| WFALM [cite/text/cf:@wfalm]                          | affine                                     | $O(n+s^2)$                                | $O(n+s^{3/2})$              | diagonal-transition, square-root-decomposition                    | reduces memory usage of WFA by only storing $1/\sqrt n$ of fronts                |
| BiWFA [cite/text/cf:@biwfa]                          | affine                                     | $O(n+s^2)$ expected                       | $O(s)$ working memory       | diagonal-transition, divide-and-conquer                           | applies [cite/text:@hirschberg75] to WFA to get linear space                     |
| Block Aligner [cite/text/cf:@block-aligner]          | Affine; scoring matrix                     |                                           |                             | SIMD, blocks, adaptive band                                       |                                                                                  |
| TALCO [cite/text/cf:@talco]                          | Affine                                     |                                           |                             | Adaptive band; *traceback convergence*                            | Resolves trace during alignment, saving memory                                   |
| BSAlign [cite/text/cf:@bsalign]                      | Affine                                     |                                           |                             | striped SIMD, difference recurrence, (adaptive) banded            | First to implement these together                                                |
| A*PA [cite/text/cf:@astarpa]                         | unit costs                                 | $O(n)$ best case                          | $O(n)$                      | A*, *gap-chaining seed heuristic*, *pruning*, diagonal-transition | only for random strings with random errors, with $n\ll\vert \Sigma\vert  ^{1/e}$ |
| A*PA2 [cite/text/cf:@astarpa2]                       | unit costs                                 | $O(n)$ best case                          |                             | DP, A*, blocks, (incremental) band-doubling, SIMD, bitpacking     |                                                                                  |
