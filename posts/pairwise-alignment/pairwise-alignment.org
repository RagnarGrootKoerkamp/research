...
#+filetags: @thesis @survey pairwise-alignment 
#+HUGO_LEVEL_OFFSET: 1
#+OPTIONS: ^:{} num:2 H:4
...
This is Chapter 2 of my [[../thesis/thesis.org][thesis]] [cite:@thesis], to introduce the first part on Pairwise Alignment.
...
Lastly, we again note that most modern read mappers and alignment tools are
...
| [cite:@hirschberg75]                       | LCS                    | $O(nm)$                                   | $\boldsymbol{O(n)}$         | divide-and-conquer                                                | introduces linear memory backtracking                                            |
...
| Singletrack [cite:@singletrack]            | Affine                 | -                                         | $nm$ instead of $3nm$       | Only store $M$, and infer $I$ and $D$ during traceback.           | Can be applied to NW, WFA, and Suzuki-Kasahara difference recurrence.            |
