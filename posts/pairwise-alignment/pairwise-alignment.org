#+TITLE: A history of exact global pairwise alignment
#+HUGO_BASE_DIR: ../..
...
Earlier review papers are [cite/text:@kruskal83] and [cite/text:@navarro01].
...
|------------------------------------------------------+----------------------------------+-------------------------------------+--------------------------------------+-----------------------------------------------------------------+-----------------------------------------------------------------------------|
| [cite/text/cf:@nw]                                   | *edit distance*                  | $O(n^2m)$                           | $O(nm)$                              | DP^{[[NW]]}                                                         | first to solve pairwise alignment polynomially                              |
...
| [cite/text/cf:@four-russians-ed]                     | edit distance^{[[discrete-scores]]}  | $\boldsymbol{O(nm/\min(m, \lg n))}$ | $O(n^2/\lg n)$^{[[score-only]]}          | [[https://en.wikipedia.org/wiki/Method_of_Four_Russians][*four Russians*]]                                                 | best known complexity                                                       |
...
| [cite/text/cf:@ukkonen85]^{[[multiple]]}                 | edit distance^{[[fixed-indel-cost]]} | $O(ns)$^{[[expected-runtime]]}          | $\boldsymbol{O(n+s^2)}$^{[[score-only]]} | *diagonal transition^{[[diagonal-transition]]}*                     | introduces diagonal transition method for unit cost distance                |
| [cite/text/cf:@myers86]^{[[multiple]]}                   | LCS                              | $O(ns)$^{[[expected-runtime]]}          | $O(n)$                               | *diagonal transition^{[[diagonal-transition]]}*, divide-and-conquer | introduces diagonal transition method for LCS, $O(n+s^2)$ expected time     |
...
| [cite/text/cf:@lcsk-overview]                        | LCS$k$                           | $O(n + r \log l)$                   | $O(n + \min(r, nl))$                 | thresholds                                                      | applies algorithm by [cite/text:@hunt77] to LCS$k$                          |
| Edlib: [cite/text/cf:@edlib]                         | Levenshtein                      | $O(nm/w)$^{[[word-size]]}               |                                      | exponential search, bit-parallel                                | global aligner using bit-parallel from [cite/text:@myers99]                 |
| WFA: [cite/text/cf:@wfa]                             | affine                           | $O(ns)$^{[[expected-runtime]]}          | $O(n+s^2)$^{[[score-only]]}              |                                                                 | $O(n+s^2)$ expected                                                         |
| WFALM: [cite/text/cf:@wfalm]                         | affine                           | $O(n+s^2)$                          | $O(n+s^{3/2})$^{[[score-only]]}          |                                                                 |                                                                             |
| biWFA [unpublished]                                  | affine                           | $O(ns)$                             | $O(n)$                               |                                                                 | $O(n+s^2)$ expected                                                         |
| A* pairwise aligner [unpublished]                    | unit cost                        | $O(n)$ expected^{[[astar-bound]]}       | $O(n)$                               | *A**, *seed heuristic*, *pruning*                               | only for random strings with random errors                                  |
...
4. <<SWG>> Smith-Waterman-Gotoh (SWG), refers to the gap-affine global alignment
   ...
   local alignment.
5. <<score-only>> When only the score is needed, and not an alignment, these
   methods only need $O(n)$ memory.
6. <<bugfix>> [cite/text/c:@altschul] fixes a bug in the backtracking algorithm of [cite/text:@gotoh].
7. <<fixed-indel-cost>> Needs all indel costs $\delta(a, -)$ and $\delta(-,b)$
   to be equal.
8. <<gap-affine>> [cite/text/c:@waterman] introduces arbitrary gap costs
...
9. <<diagonal-transition>> [cite/text/c:@ukkonen85] and [cite/text:@myers86]
   independently introduced the diagonal transition method in parallel.
10. <<astar-bound>> Needs $n\ll |\Sigma|^{1/e}$.
11. <<expected-runtime>> These methods run in $O(n+s^2)$ expected time, even
    though not all authors note this. However, the proof of [cite/text:@myers86]
    applies for all of them. [[ns2-expected][Details here]].
11. <<word-size>> $w=64$ is the word size.
