...
This algorithm ([[https://en.wikipedia.org/wiki/Needleman%E2%80%93Wunsch_algorithm][wikipedia]]) defines $D(i,j)$ as the score of the best path ending with a
...
forwards. Figure [[fig:nw]] shows the dependencies in the evaluation of a single
...
apparently he was not aware of the identical formulation in [cite/text:@wagner74].
...
[cite/text:@sankoff] introduces essentially the same quadratic recursion as [cite/text:@sellers], but
instead of minimizing a cost he gives a positive score $\delta(a_i, b_j)$ to each pair
of aligned characters, while indels have score $0$. This leads to the recursion
\begin{align}
...
When maximizing, this is a conceptually similar to computing the LCS: each pair of matching
characters increases the score. In [cite/text:@nw], there is no penalty
