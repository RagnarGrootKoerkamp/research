...
*Algorithmic improvement.* Along with the increasing length and amount of genomic sequences, alignment
...
focus shifted more towards more efficient implementations of these methods [cite:@.
...
to 512-bit operations, providing another up to $8\times$ speedup [cite:@suzuki-kasahara].
...
[cite:@minimap] (TODO better citation), that both significantly reduce the region of the DP
...
starting point and improve it to the /gap-chaining seed heuristic/ with /inexact
matches/. For inputs
...
([[*Problem Statement]]). Then, we list a number of variants of global alignment ([[*Variants of pairwise alignment]]).
