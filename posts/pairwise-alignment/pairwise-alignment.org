...
TODO: mention that ukkonen85 was mostly in 83 conference, and myers86 was
submitted in 85
...
| WFA: [cite:@wfa]                                     | affine                           | $O(ns)$^{[[expected-runtime]]}          | $O(n+s^2)$^{[[score-only]]}              |                                                                 | $O(n+s^2)$ expected                                                         |
| WFALM: [cite:@wfalm]                                 | affine                           | $O(n+s^2)$                          | $O(n+s^{3/2})$^{[[score-only]]}          |                                                                 |                                                                             |
| biWFA: [unpublished]                                 | affine                           | $O(ns)$                             | $O(n)$                               |                                                                 | $O(n+s^2)$ expected                                                         |
| A* pairwise aligner                                  | unit cost                        | $O(n)$ expected^{[[astar-bound]]}       | $O(n)$                               | *A**, *seed heuristic*, *pruning*                               | only for random strings with random errors                                  |
...
7. <<fixed-indel-cost>> Needs all indel costs $\delta(a, -)$ and $\delta(-,b)$
   to be equal.
8. <<gap-affine>> [cite/text/c:@waterman] explores non-linear cost
