...
- [cite/t:@hadlock77] introduces a /detour/ based algorithm that is essentially
  ...
  On grid graphs, the detour cost of any path is integral, and it suffices to do
  a BFS, starting with vertices with $0$ detour, than $1$ detour, and so on.
  This is easily implemented using two stacks.
