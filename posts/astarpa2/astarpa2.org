...
TODO: Drop fixed-h box from here and add to doubling figure instead.
...
A*PA2-simple starts to slow down because of doubling at lower divergence already.
...
* Conclusion
** Summary
- TODO times faster on long sequences, and TODO times faster on shorter 10%
  divergence sequence.
** Limitations
- Initialization takes time
- BiWFA is better when edit distance is /very/ low.
** Future work
- 'Smoothen' the divergence curve, choosing better thresholds for doubling, e.g.
  based on an initial heuristic alignment.
...
#+name: real-params
#+caption: Running time of A*PA2-full (left) and A*PA2-simple (middle, right) with one parameter modified. Default parameters are
