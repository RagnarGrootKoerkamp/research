...
6. *A** *.* We apply the gap-chaining seed heuristic of A*PA [cite:@astarpa]
   ([[intro]]gh), and improve it using /pre-pruning/. This technique discards most of
   the /spurious/ (off-path) matches ahead of time.

#+name: intro
#+caption: Alignment of two sequences of length $3000$bp with 25% errors using different algorithms.
#+attr_html: :class equal-width
| [[file:imgs/intro/2_dijkstra.png]] | [[file:imgs/intro/3_diagonal-transition.png]] | [[file:imgs/intro/0_gap-gap.png]] | [[file:imgs/intro/0_g-gap.png]] | [[file:imgs/intro/0_bitpacking.png]] | [[file:imgs/intro/6_astarpa2_simple.png]] | [[file:imgs/intro/7_astarpa2_full.png]]         | file:imgs/intro/5_astarpa-prune.png |
|                                | + DT                                      | + band doubling               | $\g(v_s,u){+}\cgap(u,v_t)$  | +bitpacking                      | + blocks                              | + GCSH                                      |   A*                                |
...
to obtain near-linear runtime when errors are uniform random ([[intro]]h).
...
This was then applied this with /inter-sequence parallelism/ to align a
given query to multiple reference sequences in parallel
[cite/t:@alpern95;@baeza-yates-gonnet92;@wu92;@hyyro05-increased;@rognes11].  [cite/t:@hughey96] was the first to note that
...
the bitpacking of [cite/t:@myers99] ([[intro]]e).
...
3. $\{u: \cgap(v_s, u) + \cgap(u, v_t) \leq t\}$ the static set of states possibly on a path
   of length $\leq t$ ([[intro]]cd) [cite:@ukkonen85].
4. $\{u: \g(u) + \cgap(u, v_t) \leq t\}$, as used by Edlib ([[intro]]def) [cite:@edlib;@spouge91;@papamichail2009].
5. $\{u: \g(u) + h(u) \leq t\}$, for any admissible heuristic $h$, which we will
   use as is similar to A* ([[intro]]gh).
...
$v$, and $f_l(v) = \g(u) + \cgap(u,v)+h(v) > t + 2D$, then $\f(v') > t$ when $d(v,v') \leq D$.
...
states can lie on a path of length $\leq t$ by Lemma 2. The last row is found in the same
...
* TODO
- Rerun with =debug = false=, =strip = true=, =lto = true=, =codegen-units=1=.
- update tables
- subcaptions
- Run BSAlign?
