...
2. $\{u: \g(u)\leq t\}$, the states at distance $\leq t$ [cite:@fickett84].
3. $\{u: \cgap(v_s, u) + \cgap(u, v_t) \leq t\}$ the static set of states possibly on a path
   of length $\leq t$ [cite:@ukkonen85].
4. $\{u: \g(u) + \cgap(u, v_t) \leq t\}$, as used by Edlib [cite:@spouge91;@edlib].
5. $\{u: \g(u) + h(u) \leq t\}$, for any admissible heuristic $h$.
...
computing the range of rows for each column individually, we compute it once and
then use this one range for a block of $B$ consecutive columns. While this
computes some extra states in most columns, the time saved by only determining
the range to compute once per block is larger.
...
See [[*Determining the rows to compute]] for details on exactly which rows are computed.
...
When the number of remaining lanes of rows to be computed is $\ell$, we
process $8$ lanes in parallel as long as $\ell\geq 8$. If there are remaining
rows, we end with another $8$-lane ($5\leq \ell<8$) or $4$-lane ($1\leq \ell\leq 4$)
...
do not use padding and instead fall back to trying a $4$-lane SIMD ($\ell\geq 4$),
...
#+name: ranges
#+caption: *Detail of computed ranges.* Coloured cells are invocations of $f(u) = g(u) + h(u)$. Red: $f(u) > t$, green: $f(u) \leq t$ and $u$ is fixed, and blue: $f(u)\leq t$, but only tentatively. Vertical black rectangles indicated fixed states, and blue rectangles indicate the range of rows $[j_{start}, j'_{end}]$ that must be computed for each block. The third block has no fixed states in its right column, indicating that $t$ must be increased.
#+attr_html: :class small
