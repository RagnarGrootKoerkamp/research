#+title: [WIP] A*PA2: 20x faster exact global alignment
#+HUGO_SECTION: posts
...
edit distance.  Like Edlib, A*PA2 uses Ukkonen's band doubling in combination
...
traceback based on diagonal transition, and 5) applies and improves the heuristics
developed in A*PA using pre-pruning.
...
2. *Gap-chaining seed heuristic (GCSH).* The strongest heuristic that requires
...
#+name: real-params
#+caption: Running time of A*PA2-simple (left, middle) and A*PA2-full (right) with one parameter modified. Default parameters are
