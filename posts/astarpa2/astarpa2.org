#+title: [WIP] A*PA2: up to 20x faster exact global alignment
#+HUGO_SECTION: posts
...
operations is called the /Levenshtein distance/ or /edit distance/ $s$
...
where $n=|A|$ is the sequence length.
...
previous paper [cite/t:@astarpa].

*** Needleman-Wunsch
This problem has classically been approached as a dynamic
programming problem. For string lengths $n$ and $m$, [cite/t:@nw]
...
only gave a bound of $O(nm \log (nm))$.] ([[intro]]a), where $s$ is the edit distance between
...
suffixes. A*PA [cite:@astarpa] improves the /seed heuristic/ [cite:@astarix-2] to the /gap-chaining seed heuristic/ with /pruning/
...
TODO: REWRITE THIS TO SIMPLE NOTATION AND MOVE TO PRELIMS.
...
computation to only those states that can be on a path of some small length at most $t$
through the graph ([[intro]]c). When the sequences have the same length ($n=m$), this only
...
advancements in implementation and hardware. This resulted in a plethora of new
methods. We focus here on methods for edit distance.
Many recent methods optimize for arbitrary scoring schemes and affine costs, but we do not
discuss these in detail here [cite:@sw;@gotoh;@bergeron02;@suzuki-kasahara;@bsalign].
...
[cite/t:@hughey96] was the first to note that
...
[cite/t:@myers99] introduces a /bitpacking/ algorithm specifically
for edit distance ([[intro]]f). It bit-encodes the differences between $w=64$ states in a
column into two computer words, and an efficient algorithm to operate on them.
...
instructions (see [[*Bitpacking]]) we did not pursue this further.


*** Tools
There are multiple semi-global aligners that implement $O(nm)$ (semi)-global
...
[cite:@swps3], SSW library [cite:@ssw-library] ([[https://github.com/mengyao/Complete-Striped-Smith-Waterman-Library][link]]), and KSW2 [cite:@suzuki-kasahara;@minimap2].
...
the bitpacking of [cite/t:@myers99] ([[intro]]d).
...
Recently [cite/t:@bsalign] provided a new implementation of band doubling that
also supports edit distance.
...
Further we write
$\g(u) := d(v_s, u)$ for the distance from the start,
$\h(u) := d(u, v_t)$
for the distance to the end, and $\f(u) := \g(u) + \h(u)$ for the minimal cost
of a path through $u$.
...
* Discussion
We have shown that A*PA2 is $19\times$ faster than other methods when
aligning $>500$kbp ONT reads with $6\%$ divergence, $5.6\times$ faster for
sequences of average length $11$kbp, and only slightly slower than BiWFA for
...
sequences, nearly closing the gap between approximate and exact methods.
A*PA2
...
* TODO
- General: it looks like "Previous work" was written before the previous parts of the intro since a lot of terms are defined here that should be defined earlier in the intro.
- Intro:
  - the first time you mention it, say that DP stands for dynamic programming. give a brief introduction into what it is to remind the reader what a DP matrix is (needed for later)
  - Try to replace quoted text with shorter paraphrases
  - When bringing up new methods, try to give pros and cons for each one.
- subcaptions
# Run BSAlign
