...
** Interleave
One other observation is that the first few layer are CPU bound, while the last
few layers are memory throughput bound. Thus, maybe we could process two batches
of queries at the same time but offset by half the layers and interleave them
(so that level $i$ and $i+height/2$ get processed at the same time).
I implemented this, but unfortunately the result is underwhelming.

Maybe we need to do more fine grained interleaving, and interleave operations
from /all/ levels. But that sounds really annoying to implement.

Or maybe we can split the work as: interleave the last level of one half
with /all but the last/ level of the other half? That turns out slower in
practice.

What does give a small speedup: process the first *2* levels of the next batch
...
messy. So we'll leave this as something that's more of theoretical than
practical interest, and not use it going forward.
...
** Reverse offsets
*TODO*: Fix existing benchmarks to be forward by default.
