...
- Bernoulli per-character :: Loop over the characters, and for each character apply a
  mutation with probability $1/e$.
- Exponential per-character :: A character can be mutated multiple times. Use an
  exponential distribution with parameter $e$ for the number of mutations.
