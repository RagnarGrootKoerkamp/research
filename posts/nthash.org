...
* NtHash
NtHash [cite:@nthash] is a rolling has suitable for hashing any kind of text, but made for DNA originally.
...
We can easily test whether the $64$ bitvectors are independent using Guassian
Elimination.

It turns out that just replacing $h(T)$ with the xor of the other characters
results in a matrix of rank $63=64-1$, which is not invertable and has collisions.
...
binary matrices are invertable with probability at least $28\%$. Our matrices
...
h(G))$ result in an invertable matrix is at least $10\%$ or so.
