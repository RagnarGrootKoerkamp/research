...
** Benchmarking setup
The benchmarking was done using =rustc 1.85.0-nightly= compiled with the =-r= flag. The CPU is an AMD
...
Without prefetching, we see gains up until batch size of 16 and then the gains level out.
