...
search. It is however worse at the bottom of the tree, where subsequent memory queries are
very far apart.
...
When it comes to memory throughput, Eytzinger is more advantageous. This is because the more-accessed top levels of the
...
We try at first with our original benchmarking setup; we take the first X 16-mers, sort them and work on them. The results are really strange:
...
Overall, we found that the conclusions from the Algorithmica article and from the array layouts paper hold even for batched settings. Eytzinger
...
on some platforms? If you know, please let me know.
