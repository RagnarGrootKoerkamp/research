#+title: Binary search variants and the effects of batching
#+filetags: @walkthrough binary-search data-structure
#+OPTIONS: ^:{} num:t
...
time per query (lower is better).
...
to start prefetching it. This reduces memory latency, because if the branch predictor
is correct, waiting for this prefetched value will not take such a long time.
When we use a =cmov=, there is no branch, and so the branch predictor can't do this.
...
at the top of the tree (one fetched cacheline of 64B contains 4 layers there) while sorted array
...
For us, when conducting many queries, caching the top should also be better, because it can be better reused and leads to less main
