...
TODO perf here
It now makes sense that the naive binary search has bumps on the graph. The compiler is capable of making the code partially branchless;
it replaces the conditional branch with a =cmov=, but it cannot yet make the code take a fixed number of iterations. Whether
...
But why is the code so much faster than a naive implementation or the standard library version?
TODO perf here
...
In the plot, we also include one of the fast S-tree versions to have a direct comparison on my machine. We can see that due to the use of SIMD, S-trees
are still much much faster than either Eytzinger or binary search. This is to be expected, because they can do essentially four layers of the search in a single
...
due to more non-vectorized comparisons compared to both binary search and Eytzinger.
