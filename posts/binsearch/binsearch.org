...
an =Err= or an =Ok= value, and the index inside each of those might be different.
...
We see that the non-prefetched version with a large batch size is a few percent faster on large input sizes.
...
We see that Eytzinger is quite a lot faster for both large and small sizes. The reason for this could be that Eytzinger is a lot more
...
So, we have come quite a long way! From one query taking roughly 1100ns to just about 100ns a query. That is a speedup of 11x, and it was essentially for free!
...
Keep in mind that when multithreading gets involved, the results are going to be much more noisy.
