...
we will try how much batching can improve performance of the search schemes.
...
that the batching effectively hides the memory latency anyway and the prefetching only adds unnecessary memory
...
at the higher levels of the search.
