#+title: Releasing Rust SIMD binaries to GitHub, BioConda, and PyPI
#+filetags: @devops rust simd software highlight
#+OPTIONS: ^:{} num: num:t
...
This post collects the GitHub and BioConda CI configurations I use for
maintaining and releasing Sassy. This includes releasing to:
...
We also have some specific settings to ensure the distributed binaries use [[#avx2][AVX2
SIMD instructions]], as well as some configuration to distribute x86-64 binaries
that transparently switch to a AVX-512 enabled implementation when possible.
See also this earlier post on [[../distributing-rust-simd-binaries.org][distributing Rust SIMD binaries]].
(*TODO*: Write on cargo-multivers for shipping AVX-512 compatible binaries. See
[[https://github.com/RagnarGrootKoerkamp/sassy/issues/64][this issue]] for a tricky fix.)
  
Source files for all snippets can be found at [[https://github.com/RagnarGrootKoerkamp/research/tree/master/posts/release-flow/][github:RagnarGrootKoerkamp/research/posts/release-flow]].
...
*TODO*: I came across this [[https://github.com/epage/_rust/tree/main][template repository]] by Ed Page. Pretty sure we can
learn some things from that.
...
*TODO*: This config can also be set directly in =Cargo.toml=.
...
#+caption: GitHub CI config for building and running =cargo test= on x86-64 Ubuntu and aarch64 MacOs.
#+include: "./github-release.yaml" src yaml
