...
[[https://twitter.com/nomad421/status/1701593870734336290][+twitter+ Rob]] asked for rust a implementation of [[https://github.com/jermp/pthash][PTHash]] [cite:@pthash;@pthash-2], and also
...
- Possible improvements to /Greedy peeling 1/:
...
- Cuckoo hashing idea:
  - Same as above, but displace others on the fly, doing a random walk DFS until success.
...
- So only $1$ in $10$ that works initially still works at the end.
- This is problematic, because even though in expectation most will find a
  ...
  and will need more like $O(10\cdot \ln f)$ candidates, which corresponds to
  quite large $k_i$ actually ($400\cdot \ln f \sim 4000$).
- Basically, all these cuckoo/displacement strategies are really mostly needed
  for the few buckets that lie a factor $\ln f$ above the average, so if we can
  fix those the rest should be relatively OK.
- Coming back to cuckoo hashing: Each candidate $k_i$ maps the two elements to
  ...
  DFS only needs to so roughly $10$ edges in total before success, which is very
  little actually!
