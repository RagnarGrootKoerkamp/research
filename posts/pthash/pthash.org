...
- We can divide the $m$ buckets into $P$ partitions, and then the first $1/P$ of
  buckets to the first $1/P$ of slots.
...
the array. Note that this is different from the PTHash-HEM partitioning in
...
The expected number of elements per partition is indeed $n_p$, and the standard
deviation around this is only $\sqrt(n_p)$. This means that as long as we take,
say, $4\sqrt(n_p)$ additional slots per partition, the probability of a bucket
exceeding the number of slots is less than $1$ in $16000$.
...
- $s_p$ should be a multiple of the $512$ bit cache-line size to avoid sharing
  ...
  as well.
- We should either use fast-divide for the bucket partitioning, or ensure that
  there's $2^k$ buckets per partition so we can just do this via a bitshift.
...
*A fix?* So instead of making the first $30\%$ of buckets large, we can instead
distribute the large buckets, so that every third bucket is large, or such that
each partition contains a third of large buckets at its start.

One way to make every third bucket large is as follows:
...
Alternatively, we may be able to do something like this:
- First compute the bucket id as $h(x) \mm m$. Then remap elements that should
  go to large buckets by setting the last two bits of the bucket id to $0$. That
  maps some percentage of elements to buckets $0 \mm 4$ by setting the last two
  bits of the bucket id to $0$. (We could also map them to buckets $0\mm 3$ by
  doing =i/3*3=, but that requires more operations.)
