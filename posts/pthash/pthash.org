...
Looking at the flamegraph (~cargo flamegraph~), and zooming in on the hash function, we see
...
This is fixed by enabling link-time optimization: add ~lto = "thin"~ to
~cargo.toml~.

Rerunning the benchmark we get

|                  | construction (s) | construction (s) | query (ns) | query (nw) |
