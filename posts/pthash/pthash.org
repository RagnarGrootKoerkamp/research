...
3. *Custom linear:* Store $32bit$ offset, $32bit$ (or less) difference $d$, and $16bit$ deltas to
   $offset + i \cdot d$.
4. *Local EF:* Store $32bit$ offset,  $128$ bits
...
|   threads | no remap | =Vec<u32>= | =LocalEF= | =EliasFano= | =Vec<u32>= | =LocalEF= | =EliasFano= |
...
As can be seen, =LocalEF= uses only about a third of the memory of =Vec<u32>=,
