...
*Results.*
For alphabet size $\sigma=4$ and $k=1$, the anti-lexicographic SUS-anchor empirically has density
$<1\%$ away from the density lower bound,
significantly improving over both bd-anchors that are often $>15\%$ above it and
all other minimizer schemes that have $>6\%$ overhead it for $w\leq 32$.
For alphabet size $\sigma=2$, the density is at most $10\%$ above the lower
bound, which again improves over the $>50\%$ overhead of bd-anchors and $>15\%$
overhead of all other schemes for $w\leq 32$.
...
character-based order and do not directly work with variable-length strings.)
*Vigemers* [cite:@vigemers] are also a character-based order, where $O_i$ is the
  ...
  \begin{algorithmic}[1]
    ...
    \For{$t \gets 0, 1, \dots, n-1$}
        ...
        \If{$p + \ell - 1 = t \textbf{ or }\bigl(|A| > 1\textbf{ and }A.\textsf{index}(1).x \le t+1\bigr)$}
            ...
            \Comment{Pop from $A$ if window ends or it's overtaken.}
        \EndIf
    \EndFor
    \State \Return $S$
    \EndProcedure
...
We see that as $w$ grows, so does the optimal value of $r$.
...
In the $k=1$ setting we get the same for SUS-anchors: they can only have optimal
density if SUS-anchors never overlap.]
