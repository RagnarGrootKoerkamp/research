...
Below, we present two /direct/ methods for computing the /maximal suffix/ of each
window of a string, as this
slightly simplifies the notation (compared to computing SUS-anchors, where we
"manually" have to ensure uniqueness of the suffix)
and matches [cite:@minimal-maximal-suffix-tcs].
It trivially extends to other character-based orders such as the anti-lexicographic order.
...
We now adapt the algorithm of [cite:@minimal-maximal-suffix-tcs] to our streaming setting.
...
2. $T[\last(A)\dots) > T[t\dots)$, in which case we push $t$ and note that it
   becomes hot when $\last(A)$ falls out of the window, i.e., $x_t = \last(A)+\ell$.
3. $T[\last(A)\dots) < T[t\dots)$: $t$ "takes over" $\last(A)$ when character
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    ...
    %% \Comment{Text $T[0\dots n)$, window length $\ell$, character-based order $O$}
    ...
    \For{$t \gets 0, 1, \dots, n-1$}
        ...
        \While{$A$ is not empty}
            ...
            \If{$T[a\dots a{+}\ell) \geq_O T[t\dots t{+}\ell) $}
                \Comment{New suffix is smaller.}
                \State $x_t \gets a + \ell$
                \Comment{Hot when $a$ exits the window.}
                \State \textbf{break}
            \Else \Comment{New suffix is larger: takes over $a$ at time $x$}
