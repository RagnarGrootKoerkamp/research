...
#+filetags: @results @survey sketching hpc software highlight
#+OPTIONS: ^:{} num: num:t
...
small survey and [[https://kamimrcht.github.io/webpage/sketch.html][this blogpost]] by Camille Marchet.
...
|------------+-------------+----------+----------+---------------+---------------+-------------|
| BinDash    | bottom      |      128 |       64 |         113.3 |         0.580 |      0.9669 |
| BinDash    | bottom      |     1024 |       64 |         116.3 |         3.580 |      0.9960 |
| BinDash    | bottom      |     8192 |       64 |         138.6 |        29.450 |      0.9994 |
| SimdSketch | bottom      |      128 |        8 |           2.0 |         0.216 |      0.9655 |
| SimdSketch | bottom      |     1024 |        8 |           2.0 |         1.699 |      0.9965 |
| SimdSketch | bottom      |     8192 |        8 |           2.1 |        13.689 |      0.9993 |
| SimdSketch | bottom      |    32768 |        8 |           2.8 |        54.954 |      0.9999 |
...
| BinDash-rs | bucket      |      128 |       32 |         146.4 |         0.246 |      0.9588 |
| BinDash-rs | bucket      |     1024 |       32 |         145.8 |         0.353 |      0.9929 |
| BinDash-rs | bucket      |     8192 |       32 |         147.1 |         1.977 |      0.9987 |
| SimdSketch | bucket      |      128 |        1 |           2.0 |         0.005 |      0.9254 |
| SimdSketch | bucket      |      128 |        8 |           2.0 |         0.014 |      0.9677 |
| SimdSketch | bucket      |      128 |       16 |           2.0 |         0.015 |      0.9677 |
| SimdSketch | bucket      |      128 |       32 |           2.0 |         0.015 |      0.9678 |
| SimdSketch | bucket      |     1024 |        1 |           2.0 |         0.011 |      0.9861 |
| SimdSketch | bucket      |     1024 |        8 |           2.0 |         0.057 |      0.9945 |
| SimdSketch | bucket      |     1024 |       16 |           2.0 |         0.065 |      0.9945 |
| SimdSketch | bucket      |     1024 |       32 |           2.0 |         0.067 |      0.9944 |
| SimdSketch | bucket      |     8192 |        1 |           2.2 |         0.035 |      0.9977 |
| SimdSketch | bucket      |     8192 |        8 |           2.2 |         0.382 |      0.9994 |
| SimdSketch | bucket      |     8192 |       16 |           2.2 |         0.595 |      0.9994 |
| SimdSketch | bucket      |     8192 |       32 |           2.2 |         0.881 |      0.9994 |
| SimdSketch | bucket      |    32768 |        1 |           3.0 |         0.110 |      0.9996 |
| SimdSketch | bucket      |    32768 |        8 |           3.0 |         1.736 |      0.9999 |
| SimdSketch | bucket      |    32768 |       16 |           3.0 |         2.788 |      0.9999 |
| SimdSketch | bucket      |    32768 |       32 |           3.0 |         4.074 |      0.9999 |
| SimdSketch | bucket      |   131072 |        1 |           6.9 |         0.541 |      0.9999 |
...
- Fixed by now: BinDash's bottom sketch (middle top) was off ([[https://github.com/zhaoxiaofei/bindash/issues/23][github issue]]).
...
- Add DynamicDemiLog [cite:@dynamicdemilog] and DynamicLogLog [cite:@dynamicloglog]
- Add Sketchlib.Rust (Gemsparcl) [cite:@sketchlib-rust]
- Poppunk [cite:@poppunk] uses much smaller variable k, ie 21, 25, 29. The
  purpose is to estimate two separate parameters:
  - /accessory genome/: genomes in only one species
  - /core/: shared genome
  - $p_{accessory}$: the probability a kmer is not accessory, ie the relative size of the core
  - $\pi$: the point-mutation rate in the core
  Then
  - $p_{core} = (1-\pi)^k$ is probability that kmers in the core match
  - $p_{accessory} = 1-a$ for $a$ is accessory divergence is the probability
    that the k-mers are shared in the first place.
  - $p_{match,k} = (1-a) (1-\pi)^k$.

  Sketching for multiple values of $k$ allows estimating both these parameters.

- Both gemsparcl and dynamicdemilog and Onika [cite:@onika] use an inverted
  index (all on 16-bit elements?). 1-bit hashing breaks that idea.
