...
algorithm similar to cuckoo-hashing to find suitable pilot values,
3) the same number of /slots/ for each part, with a /single/
remap table to map intermediate positions $\geq n$ to $<n$>, encoded using
per-cacheline Elias-Fano coding, and 4) /streaming/ queries, where we use
...
3. remapping using a /single/ remap table, again simplifying lookups;
4. a remap table based on a per-cacheline Elias-Fano encoding [cite:@elias;@fano], CacheLineEF;
...
*Space lower bound.*
TODO
Given $n$ random keys in some larger universe, and a random hash function to
$[n]$, intuitively the probability that this is
an MPHF is $n!/n^n$. From this, it follows that at least
...
construction can fail if that also doesn't work. CHD [cite:@chd] uses uniform
...
queries are processed in parallel. This allows prefetching pilots from
memory, and thus increases throughput and better uses the available memory bandwidth.
...
| $\gamma(x) = \frac{255}{2^8}\cdot (x^2+x^3)/2 + \frac{1}{2^8}\cdot x$ | Bucket function controlling relative bucket sizes.                |
...
For simplicity, we fix $\varepsilon = 1/{2^8}$.
...
for which the pilots take
around 300 MB and are much larger than L3 cache. Unless otherwise mentioned,
...
# [TODO: Change to CacheLineEF for fast version also?]
...
#+name: comparison
#+caption: Performance comparison of MPHF on 300 million random string keys of uniform length between 10 and 50. Construction time is shown for 6 threads. A * indicates single-threaded timings, and the optimistic 6-fold faster time in parentheses. Near-optimal values in each column are shown in bold. The common parameters $\lambda$ (number of elements per bucket) and $\alpha$ (initial load factor) are shown separately.
