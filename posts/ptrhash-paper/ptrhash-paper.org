...
Each key $k_i$ is uniformly mapped to a part and bucket using the 'fast range'
method [cite:@fast-range], where
 $part(k_i) := \left\lfloor \frac{h(k_i)}{H} \cdot P\right\rfloor$
and bucket $bucket(k_i) := \left\lfloor \frac{h(k_i)}{H} \cdot P\cdot
B\right\rfloor$. Intuitively, the fractional hash $h(k_i)/H \in [0, 1)$ is scaled to the
...
\end{equation}
In practice, we choose $S$ to be a power of $2$, so that the $\!\bmod S$ operation
is simple.
