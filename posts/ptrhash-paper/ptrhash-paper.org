...
** TODO Bucket assignment functions
:PROPERTIES:
:CUSTOM_ID: bucket-fn
:END:
...
later ones, when only a few empty slots are left.
As a result,

The bucket assignment function $\gamma$ controls the relative expected size of
all buckets. When $\gamma(x) = x$, all buckets have the same expected size
$\lambda$.

- Function: $\beta_*(x) = x+(1-x)\ln(1-x)$
- Practical function: $\beta_\varepsilon(x) = x + (1-\varepsilon)(1-x)\ln(1-x)$.
- Implementation: for a $64$ bit number $x$, $1-x$ is the negation.
  $\log_2(1-x)$ is the negative of the number of leading zeros, which we can
  multiply by a scaling factor $\ln 2$.
