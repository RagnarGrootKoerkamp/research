#+title: [WIP] PtrHash: Minimal Perfect Hashing at the Speed of Memory
...
collision-free bijective map $\mphf$ from $K$ to $\{0, \dots, n-1\}$. These
functions have uses in databases, search engines, and are used in bioinformatics
indexing tools such as Pufferfish (using BBHash), and Piscem (PTHash).  Our main
motivating usage is that as in SSHash, where PTHash only takes $5\%$ of the
total space.  Thus, this work presents a (minimal) perfect hash function that
...
*Contributions.*
PtrHash builds on PTHash by 1) using fixed-width (uncompressed) 8-bit pilots, 2) a construction
algorithm similar to cuckoo-hashing to find suitable pilot values, 3) a /single/
remap table covering all parts, encoded using per-cacheline EF, and 4) the use
of prefetching to answer a stream of queries more efficiently than one-by-one processing.

*Results.*
We test two parameter configurations of PtrHash, /fast/ and /compact/, that
achieve 3.0 and 2.12 key. On 300 million string keys, they are as fast or faster
...
low as 12ns/key when iterating in a for loop, or even down to 8ns/key when using
the streaming approach, just short of the 7.4ns throughput of random memory accesses.
