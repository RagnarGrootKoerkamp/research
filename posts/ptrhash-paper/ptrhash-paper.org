...
Each key $k_i$ is (nearly) uniformly mapped to a part and bucket using the 'fast range'
method [cite:@fast-range], where
\begin{align}
part(k_i) &:= \left\lfloor P\cdot h(k_i) / 2^{64}\right\rfloor,\\
bucket(k_i) &:= \left\lfloor P\cdot B\cdot h(k_i)/2^{64}\right\rfloor.
\end{align}
Intuitively, the fractional hash $h(k_i)/2^{64} \in [0, 1)$ is scaled to the
...
impl PtrHash {
    fn query(&self, key: Key) -> usize {
        // Assuming a 64 bit hash.
        let h = self.hash(key) as u128;
        // This compiles to taking the high 64 bits of the 64x64->128 multiplication.
        let part = ((self.P as u128 * h) >> 64) as usize;
        let bucket = (((self.P * self.B) as u128 * h) >> 64) as usize;
        let pilot = self.pilots[bucket];
        let x = h ^ self.hash_pilot(pilot);
        let reduced = ((self.C as u128 * x as u128) >> 64) as usize & (self.S - 1);
        let slot = (part << lgS) + reduced;
        if slot < self.n {
            return slot
        } else {
