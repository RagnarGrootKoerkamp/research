...
| $\lambda=3.5$                             | Expected number of elements per bucket.                           |
...
| $\gamma: [0,1)\mapsto [0,1)$               | Bucket function controlling relative bucket sizes.                |
...
#+name: bucket-fn
#+caption: The left shows various bucket assignment functions $\gamma$, such as the piecewise linear function used by FCH (TODO CHECK) and PTHash, and the optimal function introduced by PHOBIC. Flatter slopes at $x=0$ create larger buckets, while steeper slopes at $x=1$ create more small buckets, as shown on the right, as the distribution of relative expected bucket sizes given by $(\gamma^{-1})'$.
| [[file:bucket-fn.svg]] | [[file:bucket-size.svg]] |
...
assumption that bucket sizes are continuous.] function
...
slope of $\gamma$ is at least $\varepsilon=1/(5\cdot \sqrt B)$ by using
...
\frac{127\cdot (x^2+x^3)/2 + 1\cdot x}{128}$.
...
Intel Skylake CPUs can execute over $200$ instructions ahead while waiting for memory
...
each iteration to allow pipelining to fetch memory more iterations ahead.
...
This way, no additional on-disk memory is needed for construction.
