...
#+name: comparison
#+caption: Performance comparison of PtrHash against other methods, on 300 million random string keys of uniform length between 10 and 50. Parameters are based on Table 1 of [cite:@phobic] and Table 3 of [cite:@fmph], with configurations that are slow to construct (notably, bipartite ShockHash-RS) omitted. Construction time is shown when using 6 threads, apart from CHD which does not support parallel construction. Near-optimal values in each column are shown in bold. The common parameters $\lambda$ (number of elements per bucket) and $\alpha$ (initial load factor) are shown separately.
#+attr_html: :class small
| Method                              | $\lambda$ | $\alpha$ | Space@@html:<br/>@@(bits/key) | Construction@@html:<br/>@@ (6t, ns/key) | Query @@html:<br/>@@(ns/key) |
| CHD                                 |       3.0 |          |                          2.27 |                                   1059* |                          542 |
...
| SicHash, $p_1=0.21$, $p_2=0.78$     |           |     0.90 |                          2.41 |                                      48 |                          149 |
| SicHash, $p_1=0.45$, $p_2=0.31$     |           |     0.97 |                          2.08 |                                      63 |                          141 |
| PTHash C-C                          |       4.0 |     0.99 |                          3.19 |                                     403 |                           77 |
| + HEM                               |           |          |                               |                                     173 |                              |
| PTHash EF                           |       5.0 |     0.99 |                          2.17 |                                     765 |                          156 |
| + HEM                               |           |          |                               |                                     323 |                              |
| PHOBIC IC-C                         |       3.9 |      1.0 |                          4.14 |                                      62 |                          116 |
| PHOBIC IC-R                         |       4.5 |      1.0 |                          2.34 |                                      80 |                          179 |
| PHOBIC IC-R                         |       6.5 |      1.0 |                        *1.94* |                                     215 |                          163 |
| PHOBIC IC-R                         |       7.0 |      1.0 |                        *1.86* |                                     446 |                          157 |
| PtrHash fast, Vec                   |       3.0 |     0.99 |                          2.99 |                                    *26* |                         *38* |
| + streaming                         |           |          |                               |                                         |                         *20* |
| PtrHash compact,  CLEF              |       4.0 |     0.99 |                          2.12 |                                      62 |                         *42* |
| + streaming                         |           |          |                               |                                         |                         *23* |
...
CHD [cite:@chd], FMPH and FMPHGO [cite:@fmph], SIMDRecSplit
[cite:@recsplit;@recsplit-gpu], SicHash [cite:@sichash], PTHash
[cite:@pthash;@pthash-2], and PHOBIC [cite:@phobic].
We omit Bipartite ShockHash-RS
[cite:@shockhash;@bipartite-shockhash] and some other parameter configurations that yield smaller space but
take significantly longer to construct.
These
results were obtained using the excellent MPHF-Experiments library
[cite:@mphf-experiments] by Hans-Peter Lehmann. Constructing is done on 6
...
alongside SIMDRecSplit, in 26 ns/key, resulting in just below 3 bits/key for
both. However, PtrHash is $8\times$ faster to query, going up to $15\times$ when
streaming all queries at once.
...
*Future work.*
We see a few directions for future work. First, while it seems that query
throughput is at the limit, it may be possible to increase it even further by
using more skewed bucket size distributions. The larger buckets them cover a
larger part of the keys, and thus more queries can be answered from cache.
Possibly this could be done by allowing a small number of collisions between
keys in a bucket, and separately fixing those.

The size of the list of pilots could possibly be improved by further parameter
tuning. In particular, we only tried 8-bit pilots, while slightly fewer or more
bits may lead to smaller data structures. An experiment with 4-bit pilots
was not promising, however.
Further, we did not do a theoretical analysis of expected length of the eviction
chains and the running time of the construction. Such an analysis could help to
better understand the optimal bucket assignment function for this algorithm,
like PHOBIC [cite:@phobic] did for the case without eviction.
...
specifically, it is more efficient when the strings are consecutive in memory
rather than separately allocated. Attention could also be given to efficiently
hashing many strings in parallel.
