...
- [cite:@assembly-graph-fm] computes $One-to-All(i)$ in $O(n)$ using FM-index,
  but can be modified to only report irreducible edges. It skip overlaps induced
  by the transitive closure. Can we do something similar for $One-to-All$ or $Top$?
