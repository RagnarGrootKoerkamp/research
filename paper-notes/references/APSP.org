...
All are $O(n)$ memory.
|-----------------+-------------------+-----------------------------+--------------------------+------------------------+--------------------------|
| query           | construction | query time (paper)          | note                     | new query time  | note                     |
|-----------------+-------------------+-----------------------------+--------------------------+------------------------+--------------------------|
| One-to-One(i,j) | $n \log \log k$   | $\log \log k$               | SuffixTree + Rank-Select | -                      |                          |
| One-to-All(i)   | $n$               | $k$                         | Aho-Corasick             | $k$                    | replace $\tau$ tree |
...
- [cite:@assembly-graph-fm] computes the string overlap graph in $O(n)$ and
  skips overlaps induced by the transitive closure.
