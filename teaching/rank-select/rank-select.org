...
#+begin_definition 
A /rank/ query on a bit vector counts 1-bits /before/ a position $i$ (for $0\leq
i\leq n$).
...
#+end_definition
#+reveal: split:t
#+begin_definition "(Clark's select ctd.) handling dense blocks"
...
#+begin_definition "(Clark's select ctd.) queries"
To answer $\select(j)$:
...
#+begin_theorem 
There exists a data structure with $o(n)$ space overhead that allows $O(1)$ rank
and select queries.
#+end_theorem
...
- What is the main technique used to obtain a succinct rank data structure? And
  for select?
- Name a few ways in which practical rank methods differ from the Jacobson's
  theoretical version.
- How does Darray differ from Clark's select?
- Which modern CPU instructions can be used for rank and/or select? What older
  techniques do they replace?
- For both rank and select, why does a single layer of blocks not suffice to
  obtain $o(n)$ space and $O(1)$ query time?

* Blackboard: Bit vector & Rank
#+attr_html: :class blackboard :src /ox-hugo/blackboard-rank.jpg
[[file:./blackboard-rank.jpg]]
* Blackboard: Select & Darray
#+attr_html: :class blackboard :src /ox-hugo/blackboard-select.jpg 
[[file:./blackboard-select.jpg]]
