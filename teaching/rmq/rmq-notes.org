...
needed to answer queries $\rmq(i, i+2)$.
...
We can use $\rmq(0,n)=i$ to find the smallest element in the array.
Then, we can use $\rmq(0, i)$ to find the smallest element left of it (if
$i>0$) and $\rmq(i+1, n)$ to find the smallest element right of it (if $i < n$).
...
- Store a tree of $k$ levels $0, \dots, k$.
- In level $\ell$, store $n+1-2^\ell$ values for $0\leq i < n+1-2^\ell$,
  each storing the position of the minimum over a (sliding) /window/ of length $2^\ell$:
  $$S_\ell[i] := \argmin_{i\leq j < i + 2^\ell} (a_i).$$
  - (Level $0$ simply stores $S_0[i] = i$, and could be omitted.)
- To answer a query $[l,r)$, first find the largest power $2^\ell$ that is at
  ...
  $[l, r)$. Thus, we get:
  $$\argmin_{l\leq i< r} (a_i) = \argmin_{i\in \{S_\ell[l], S_\ell[r-2^\ell]\}} (a_i).$$

- If only the /value/ of the minimum is needed, that can be stored directly,
  instead of its index. Then the last step simply takes the minimum of the two values.
...
- Then:
  - if $l\bmod 8 = 4$, use $S_2[l/4]$ and increment $l$ by $4$.
  - if $r\bmod 8 = 4$, use $S_2[r/4-1]$ and decrement $r$ by $4$.
