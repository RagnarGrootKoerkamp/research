...
* Sparse Table: $O(1)$ queries, $n \log_2 n$ words
...
- Level $\ell$ has $\lfloor n/2^\ell\rfloor$ nodes, each storing the
  ...
  $$S_\ell[i] := \min_{i\cdot 2^\ell \leq j < (i + 1)\cdot 2^\ell} a_i.$$
