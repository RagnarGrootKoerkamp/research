...
Given two genomes $A$ and $B$, our method approximates the average hamming
...
Then, we find all pairs of positions in $A$ and $B$ where this spaced seed
induces spaced k-mer matches.
Our estimate is the average hamming distance between matching positions.
