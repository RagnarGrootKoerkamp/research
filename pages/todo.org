...
- WFA with /diamond optimization/ ::
  Instead of defining WFA fronts as those of constant $s$, the search can be
  directed towards the target diagonal: compute /layers/ of /rectangles/. It
  turns out this does not work well in practice because it's hard to
  parallelize. A writeup on the details would be nice.
- Match score for asymmetric and dual affine costs :: As mentioned in the
  =WFAlm= paper by Eizenga and Paten, a match score can be handled in the
  cost-only setting by instead increasing the cost of mismatches and extends.
  This idea generalizes to asymmetric costs and dual affine costs. Further, it
  can be modified to not have half-integer costs: instead of effectively adding
  cost $l/2$ per processed character, we can add cost $l$ each time a character
  of $A$ is processed (that is, for mismatches and deletion-extend edges). By
  adding it in the direction of the shorter sequence, the final score will be
  lower. Some investigation is needed to see whether adding score $l$ to
  characters of $A$ or $B$ actually makes a significant difference.
...
- /Diamond optimization/ for WFA ::
  We spent some time trying to implement this, but then realized it can never
  work efficiently because of dependencies between consecutive states. A short
  write-up on this would be nice.
...
- Test/bench datasets ::
  ...
  - random strings with 1 long indels
  - random strings with multiple long indels
  - repeating strings, but with a different number of repeats
  - repeating strings, but with additional mutations (both $A$ and $B$ are $e/2$
    mutations away from the pattern). May also have a different number of repeats.
  ...
  - Random with different length distribution on indel length, so they are
    longer than the defaul geometric distribution.
