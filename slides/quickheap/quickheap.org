...
* "Solved" problem: Brodal heaps are "optimal"
- $\push$: $O(1)$ worst-case
...
# Binary heaps: the default, but cache misses and no SIMD
...
- $\pop$ randomly walks down the tree:
  ...
  - 80ns cache miss!
  - Not good for SIMD
# Dropped I/O complexity here
  # - $O(\log_2 n)$ 
  # - a random access / cache miss
...
#+attr_html: :class medium :style top:65% :src /ox-hugo/slides-heaps.svg
[[file:slides-heaps.svg]]

* Avoiding cache-misses is possible!
:PROPERTIES:
:CUSTOM_ID: good-io-complexity
:END:

Move a cache line worth of elements at a time into/out of RAM, only using
sequential scans.
...
* The QuickHeap: inspired by quick select
:PROPERTIES:
:CUSTOM_ID: quickheap
:END:

# original for in-place online bottom-k
...
#+attr_html: :class medium :style top:65% :src /ox-hugo/slides-quickheap.svg
[[file:slides-quickheap.svg]]
...
#+attr_html: :class medium :style top:60% :src /ox-hugo/slides-simdquickheap.svg
[[file:slides-simdquickheap.svg]]

** Classification using SIMD
:PROPERTIES:
:CUSTOM_ID: classification
:END:
- Quickly count the number of pivots $>x$.
#+attr_html: :class medium :style top:60% :src /ox-hugo/quickheap-classification.svg
[[file:quickheap-classification.svg]]
** Partitioning using SIMD
:PROPERTIES:
...
#+attr_html: :class medium :style top:65% :src /ox-hugo/quickheap-partitioning.svg
[[file:quickheap-partitioning.svg]]
